...
- Если Google Docs не создан:
  ...
  - Укажи `url` и `file_id` **как в ответе инструмента** (путь от корня, вида `/flows/api/v1/files/download/...`). Не придумывай и не подставляй `http://localhost`, `127.0.0.1` и другие origin — ссылка открывается в том же origin, что и приложение; отдельную строку «полный URL с localhost» не добавляй.
