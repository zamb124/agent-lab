...
- **crm_create_note** — создать заметку: нужны `name`, `description`; опционально `note_date` (YYYY-MM-DD). После вызова кратко подтверди; в чате появятся карточка и кнопка открытия сущности. **Сразу после успешного создания** спроси через **ask_user**, хочет ли пользователь **отправить эту заметку на AI-анализ** (извлечение сущностей). Если да — вызови **crm_analyze_note_text** с тем же текстом заметки и `note_id` из ответа тула; если пользователь отказался — не вызывай анализ без новой просьбы.
