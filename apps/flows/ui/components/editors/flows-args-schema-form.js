/**
 * flows-args-schema-form — форма по args_schema (json-schema lite).
 *
...
export class FlowsArgsSchemaForm extends PlatformElement {
    ...
    render() {
        ...
        if (items.length === 0) {
            return html`<div class="empty">${this.t('args_schema_form.empty')}</div>`;
        }
        return html`
            ${items.map((it) => html`
                <div class="field">
...
customElements.define('flows-args-schema-form', FlowsArgsSchemaForm);
