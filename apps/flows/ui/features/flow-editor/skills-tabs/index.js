 ...
 * Реализует полную логику FlowFactory._apply_skill с backend
 */
...
export class SkillsTabsBar extends PlatformElement {
    ...
    willUpdate(changedProperties) {
        const flowConfigChanged = changedProperties.has('flowConfig') && this.flowConfig;
        const currentSkillIdChanged = changedProperties.has('currentSkillId');
        
        if (flowConfigChanged) {
            this._initSkills(this.flowConfig);
        ...
        // Если изменился только currentSkillId (без flowConfig) - переключаемся
    ...
    _initSkills(flow) {
        this._baseData = {
            ...
            variables: this._extractVariableValues(flow.variables || {}),
        };

        this._skillsData.clear();

        if (flow.skills) {
            for (const [skillId, skill] of Object.entries(flow.skills)) {
