    ...
    *,
    ...
    async def write_episode() -> None:
        try:
            ...
            _ = await store.write_episode(episode_to_write)
            latest_cursor_map = dict(state.llm_context_memory_cursor)
            latest_cursor_map[key] = max(latest_cursor_map.get(key, 0), next_cursor)
