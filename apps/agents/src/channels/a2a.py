...
from apps.agents.src.evaluation.service import EvaluationService
from apps.agents.src.files import FileHandler
from core.logging import get_logger
from apps.agents.src.services.push_notifications import dict_to_config
from apps.agents.src.streaming import Emitter
from core.state import ExecutionState
from apps.agents.src.streaming.subscriber import EventSubscriber, StreamEvent
from apps.agents.src.tasks.push_notification_tasks import (
    delete_config,
    get_config,
    list_configs,
    send_task_update,
...
class A2AChannel(BaseChannel):
    ...
    async def _run_evaluation(
        ...
        try:
            async for event in service.run_test_stream(
                self.agent_id, skill_id, test_case_id, task_id=task_id
            ):
    ...
    async def on_message_stream(
        ...
        logger.info(f"[on_message_stream] Starting for agent={self.agent_id}")
        prepared = await self._prepare_a2a_params(params)
