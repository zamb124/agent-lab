...
from apps.agents.src.models import AgentConfig, TriggerConfig, TriggerStatus, TriggerType
from apps.agents.src.triggers.handlers.base import BaseTriggerHandler
...
class TriggerRegistry:
    ...
    async def sync_triggers(
        ...
        for trigger_id in removed_ids:
            ...
            await self._unregister_trigger(agent_id, trigger)
        
        # Register новых
        for trigger_id in added_ids:
            trigger = new_triggers[trigger_id]
            if trigger.enabled:
                updated_trigger = await self._register_trigger(agent_id, trigger)
        ...
        for trigger_id in common_ids:
            ...
            if self._trigger_changed(old_trigger, new_trigger):
                # Перерегистрируем
                await self._unregister_trigger(agent_id, old_trigger)
                if new_trigger.enabled:
                    updated_trigger = await self._register_trigger(agent_id, new_trigger)
                    new_triggers[trigger_id] = updated_trigger
                else:
    ...
    async def register_all(self, agent_id: str, config: AgentConfig) -> AgentConfig:
        """
    ...
    async def unregister_all(self, agent_id: str, config: AgentConfig) -> None:
        """
    ...
    async def _register_trigger(
        ...
        if handler is None:
            logger.warning(
                ...
                f"agent={agent_id}, trigger={trigger.trigger_id}"
            )
        ...
        try:
            updated_trigger = await handler.register(agent_id, trigger)
            logger.info(
                f"Trigger registered: agent={agent_id}, "
                f"trigger={trigger.trigger_id}, type={trigger_type_str}"
            )
            return updated_trigger
        except Exception as e:
            logger.error(
                f"Failed to register trigger: agent={agent_id}, "
    ...
    async def _unregister_trigger(
        ...
        if handler is None:
            logger.warning(
                f"No handler for trigger type {trigger_type_str}, "
                f"cannot unregister: agent={agent_id}, trigger={trigger.trigger_id}"
            )
            return
        
        try:
            await handler.unregister(agent_id, trigger)
            logger.info(
                f"Trigger unregistered: agent={agent_id}, "
                f"trigger={trigger.trigger_id}, type={trigger_type_str}"
            )
        except Exception as e:
            logger.error(
                f"Failed to unregister trigger: agent={agent_id}, "
                f"trigger={trigger.trigger_id}, error={e}"
            )
