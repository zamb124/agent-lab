        ...
        async with self._storage._get_session() as session:
            ...
            for row in rows:
                ...
                if len(session_parts) >= 3 and session_parts[0] == "eval":
                    agent_id_from_session = session_parts[1]
                    context_id_from_session = session_parts[2]
                else:
                    agent_id_from_session = session_parts[0] if session_parts else ""
                ...
                session = SessionConfig(
                    ...
                    agent_id=agent_id_from_session,
