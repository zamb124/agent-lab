...
from apps.agents.src.models import EvaluationResult
...
class EvaluationRepository:
        ...
        async with self._storage._get_session() as session:
            query = text("""
                INSERT INTO evaluation_results
                    (agent_id, skill_id, run_date, iteration, test_case_id, task_id,
                     status, duration_ms, turns_count, dialog, scores,
                     judge_feedback, error, created_at)
                VALUES (:p1, :p2, :p3, :p4, :p5, :p6, :p7, :p8, :p9, :p10, :p11, :p12, :p13, :p14)
                ON CONFLICT (agent_id, skill_id, run_date, iteration, test_case_id)
                DO UPDATE SET
    ...
    async def get_by_run(
        ...
        async with self._storage._get_session() as session:
            if iteration is not None:
                ...
                query = text("""
                    SELECT * FROM evaluation_results
                    WHERE agent_id = :agent_id AND skill_id = :skill_id AND run_date = :run_date
    ...
    async def get_next_iteration(self, agent_id: str, skill_id: str, run_date: date) -> int:
        """
