...
from apps.agents.src.runners import PythonCodeRunner
...
async def get_code_completions(
    ...
    Args:
        ...
        perspective: Ракурс (editor, agent, tool, node)
...
async def execute_code(request: ExecuteRequest) -> ExecuteResponse:
    ...
    try:
        ...
        agent_id = request.node_config.get("agent_id", "test-agent")
...
def _validate_node_config(config: Dict[str, Any]) -> None:
    ...
    if node_type == "code":
        ...
        if not config.get("url") and not config.get("agent_id"):
            raise ValueError("url или agent_id обязателен для remote_agent")
