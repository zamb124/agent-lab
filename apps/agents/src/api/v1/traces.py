...
Позволяет получить все spans по сессии, task_id, user_id или agent_id.
"""
...
async def get_traces_by_session(
    ...
    """
    Получает все трейсы для сессии агента.
    
    Args:
        session_id: ID сессии агента (session_agent)
...
async def get_traces_by_flow(
    ...
    spans = await container.span_repository.get_spans_by_flow(
        agent_id, from_time, to_time, limit
    )
