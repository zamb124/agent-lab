...
async def submit_task(request: TaskSubmitRequest) -> Dict[str, Any]:
    ...
    if request.session_id:
        ...
        if ":" not in session_id:
            raise HTTPException(
                status_code=400,
                detail=f"session_id must be in format 'agent_id:context_id', got: '{session_id}'"
            )
        ...
        session_id = f"{request.agent_id}:{context_id}"
...
async def get_state(
    ...
    """
    Получает state по session_id или context_id+agent_id.
    
    Args:
        session_id: ID сессии (альтернатива context_id+agent_id)
    ...
    if session_id:
        state = await container.state_manager.get_state(session_id)
    elif context_id and agent_id:
        session_id = f"{agent_id}:{context_id}"
        state = await container.state_manager.get_state(session_id)
    else:
        raise HTTPException(
            status_code=400,
            detail="Need session_id or context_id+agent_id"
        )
