...
async def _get_agent_config(agent_id: str, version: Optional[str] = None) -> Optional[AgentConfig]:
    """
    ...
    if version:
        return await container.agent_repository.get_version(agent_id, version)
...
async def get_agent_card_well_known(
    ...
    """
    config = await _get_agent_config(agent_id, version=v)
    if not config:
        raise HTTPException(status_code=404, detail=f"Agent '{agent_id}' not found")
    
    context = get_context()
    channel = A2AChannel(agent_id, context=context, agent_config=config)
...
async def json_rpc_handler(
    ...
    config = await _get_agent_config(agent_id, version=version)
    if not config:
        ...
        return JSONResponse(
            {
                "jsonrpc": "2.0",
                "id": rpc_id,
                "error": {"code": -32000, "message": f"Agent not found: {agent_id}{version_info}"},
            }
        )
    ...
    handler = A2AChannel(agent_id, context=context, agent_config=config)
...
async def list_skills(agent_id: str) -> List[Dict[str, Any]]:
    context = get_context()
    channel = A2AChannel(agent_id, context=context)
...
async def get_skill(agent_id: str, skill_id: str) -> Dict[str, Any]:
    context = get_context()
    channel = A2AChannel(agent_id, context=context)
    config = await _get_agent_config(agent_id)
    if not config:
...
@router.get("/{agent_id}/skills/{skill_id}/tools")
async def get_skill_tools(agent_id: str, skill_id: str) -> List[Dict[str, Any]]:
...
async def get_skill_schema(agent_id: str) -> Dict[str, Any]:
...
async def create_skill(agent_id: str, request: Request) -> Dict[str, Any]:
    """Создать новый skill."""
    context = get_context()
    channel = A2AChannel(agent_id, context=context)
    config = await _get_agent_config(agent_id)
...
async def update_skill(agent_id: str, skill_id: str, request: Request) -> Dict[str, Any]:
...
async def delete_skill(agent_id: str, skill_id: str) -> Dict[str, Any]:
    """Удалить skill."""
    context = get_context()
    channel = A2AChannel(agent_id, context=context)
    config = await _get_agent_config(agent_id)
