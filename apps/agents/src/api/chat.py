...
async def get_chat_interface(agent_id: str, request: Request) -> HTMLResponse:
    """
    ...
    if not (template_dir / template_file).exists():
        ...
        html_content = _get_embedded_template(agent_id, base_url, skills_list)
...
def _get_embedded_template(agent_id: str, base_url: str, skills: List[dict]) -> str:
