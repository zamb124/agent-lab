"""
SimpleReactNodeExecutor - базовый класс для простых внешних агентов.
...
from apps.agents.src.models import NodeConfig, NodeLLMOverride
from apps.agents.src.models.enums import NodeType
from core.state import ExecutionState
from apps.agents.src.streaming import InMemoryEmitter


class SimpleReactNodeExecutorMeta(type(AgentExecutor)):
    ...
    def __new__(mcs, name, bases, namespace):
        ...
        node_description = namespace.get("description", f"ReAct агент {node_name}")
        ...
        def __init__(self):
            ...
            self.runner = ReactNodeRunner(config, tools, None, prompt, react_node=None)
            self.skills = skills
...
class SimpleReactNodeExecutor(AgentExecutor, metaclass=SimpleReactNodeExecutorMeta):
    """
    Базовый класс для простых внешних агентов.
    
    Просто задайте tools и prompt:
    
    class MyAgent(SimpleReactNodeExecutor):
        tools = [MyTool()]
        prompt = "Ты помощник..."
