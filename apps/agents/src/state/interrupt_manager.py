...
Работает с любой вложенностью:
- Agent → Node → Agent → Субагент → ask_user
- Agent → GraphNode → Agent → ask_user
- Agent → Node → Agent → Субагент → Суб-субагент → ask_user
...
class InterruptManager:
    ...
    - nested_states: Dict[str, Dict] состояния вложенных агентов
    """
    ...
    def save_nested_state(
        ...
        Args:
            ...
            nested_id: ID вложенного вызова (agent_id, node_id, etc)
            nested_state: State вложенного вызова
        """
        from apps.agents.src.state.execution_state import NestedStateData
