...
class FlowFactory:
    ...
    def agent_repository(self):
        if self._agent_repository is None:
            self._agent_repository = get_agents_container().agent_repository
    ...
    async def get_flow_sessions(
        ...
        all_sessions = await self.session_repository.list_all(limit=1000)
        ...
        if unique_flow_ids:
            all_flows = await self.flow_repository.list_all(limit=1000)
            flows_dict = {f.flow_id: f for f in all_flows}
            for flow_id in unique_flow_ids:
                if flow_id in flows_dict:
                    flow_cache[flow_id] = flows_dict[flow_id].name
                else:
        ...
        if unique_user_ids:
            users_dict = await self.user_repository.get_many(list(unique_user_ids))
            for user_id in unique_user_ids:
                if user_id in users_dict:
                    user_cache[user_id] = users_dict[user_id].name
                else:
    ...
    async def install_flow(self, flow_id: str) -> Dict[str, Any]:
        ...
        Args:
            flow_id: ID flow для установки (например, "apps.agents.flows.weather_flow.weather_flow_config")
    ...
    async def uninstall_flow(self, flow_id: str) -> None:
        ...
        await self.flow_repository.delete(flow_id)
