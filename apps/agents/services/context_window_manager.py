...
Пиши от третьего лица, структурированно."""
        ...
        summary_text = getattr(summary_response, "content", None) or ""
        
        if not summary_text or not summary_text.strip():
            logger.warning(
                f"⚠️ LLM вернул пустую суммаризацию для модели {summarization_model}, "
                f"возвращаем исходные сообщения без суммаризации"
            )
            return messages
