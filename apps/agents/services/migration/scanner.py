...
import apps.agents.agents as agents_pkg
import apps.agents.flows as flows_pkg
import apps.agents.flows.custom as custom_flows_pkg
...
class CodeScanner:
    ...
    async def find_agent_classes(self) -> List[Type[BaseAgent]]:
        ...
        modules_to_scan = [agents_pkg, flows_pkg, custom_flows_pkg]
    ...
    async def find_flow_ids(self) -> List[str]:
        ...
        Returns:
            Список flow_id (например, ["apps.agents.flows.weather_flow.weather_flow_config"])
        """
        flow_ids = []

        modules_to_scan = [flows_pkg, custom_flows_pkg]
