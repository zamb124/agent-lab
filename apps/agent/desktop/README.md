...
## Windows self-contained MSI

`.msi` **не требует** установки VC++ Redistributable у пользователя. CI кладёт MSVC CRT + UCRT + cargo DLL рядом с `goosed.exe` в `resources/bin/`:

- [`scripts/bundle_windows_runtime_dlls.ps1`](scripts/bundle_windows_runtime_dlls.ps1) — копирование + gate `dumpbin /dependents`
- вызывается из [`scripts/build.sh`](scripts/build.sh) после `build_goosed_binary` (platform `windows`)

Ошибка `0xC0000135` / child exit `3221225781` = missing DLL до bundling.
