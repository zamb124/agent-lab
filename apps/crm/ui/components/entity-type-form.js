...
import { normalizeSchemaRows, buildSchemaFromRows } from './schema-field-builder.js';
...
export class EntityTypeForm extends PlatformElement {
    ...
    static styles = [
        ...
        css`
            ...
            .schema-section { border: 1px solid var(--crm-stroke); border-radius: var(--radius-md); background: var(--crm-surface-elevated); padding: var(--space-3); display: flex; flex-direction: column; gap: var(--space-2); }
            .schema-section-header { display: flex; align-items: center; justify-content: space-between; gap: var(--space-2); }
            .schema-section-title { color: var(--text-secondary); font-size: var(--text-sm); font-weight: 500; font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace; }
            .schema-loading { color: var(--text-tertiary); font-size: var(--text-sm); }
            @media (max-width: 767px) { .schema-builder-grid { grid-template-columns: 1fr; } }
    ...
    static draftFromEntityType(entityType) {
        return {
            type_id: entityType.type_id || '',
            ...
            is_event: entityType.is_event === true,
            check_duplicates: entityType.check_duplicates !== false,
            is_context_anchor: Boolean(entityType.is_context_anchor),
            required_fields_rows: normalizeSchemaRows(entityType.required_fields || {}),
            optional_fields_rows: normalizeSchemaRows(entityType.optional_fields || {}),
        };
    }
    ...
    _onSave() {
        ...
        try {
            ...
            requiredFields = buildSchemaFromRows(
                this.draft.required_fields_rows || [],
                this.i18n.t('schema_sections.required_fields'),
                this.schemaOptions,
                t,
            );
            optionalFields = buildSchemaFromRows(
                this.draft.optional_fields_rows || [],
                ...
                <div class="schema-builder-grid">
                    <div class="schema-section">
                        <div class="schema-section-header">
                            <span class="schema-section-title">required_fields</span>
                        </div>
                        ${this.schemaOptions
                            ? html`
                                <schema-field-builder
                                    .rows=${this.draft.required_fields_rows || []}
                                    .schemaOptions=${this.schemaOptions}
                                    .required=${true}
                                    @rows-changed=${(e) => this._update('required_fields_rows', e.detail.rows)}
                                ></schema-field-builder>
                            `
                            : html`<div class="schema-loading">${this.i18n.t('entity_type_form.schema_loading')}</div>`
                        }
                    </div>
                    <div class="schema-section">
                        <div class="schema-section-header">
                            <span class="schema-section-title">optional_fields</span>
                        </div>
                        ${this.schemaOptions
                            ? html`
                                <schema-field-builder
                                    .rows=${this.draft.optional_fields_rows || []}
                                    .schemaOptions=${this.schemaOptions}
                                    @rows-changed=${(e) => this._update('optional_fields_rows', e.detail.rows)}
                                ></schema-field-builder>
                            `
                            : html`<div class="schema-loading">${this.i18n.t('entity_type_form.schema_loading')}</div>`
                        }
