...
class DailySummaryCacheService:
    ...
    async def get_state(
        ...
        raw_value = cast(str | None, await self._redis.get(key))
    ...
    async def set_state(
        ...
        key = self._state_key(company_id=company_id, namespace=namespace, date_str=date_str)
        await self._redis.set(key, json.dumps(dict(state)), ex=ttl_seconds)
    ...
    async def is_revalidating(self, company_id: str, namespace: str | None, date_str: str) -> bool:
        ...
        exists_count = cast(int, await self._redis.exists(key))
