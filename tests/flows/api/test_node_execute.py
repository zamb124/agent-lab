...
- ASGI тестовый сервер для external_api и remote_flow
    ...
    async def test_remote_flow_basic(self, client, app, remote_flow_server):
        ...
        response = await client.post(
            ...
            json={
                ...
                "node_config": {"url": remote_flow_server},
                "state": state
            }
        )
        assert response.status_code == 200
...
class TestFlowNode:
    ...
    async def test_subflow_basic_execution(self, client, app, container):
        ...
        response = await client.post(
            ...
            json={
                "node_type": "flow",
                "node_config": {"flow_id": "example_graph"},
                "state": state
            }
        )
        assert response.status_code == 200
    ...
    async def test_subflow_nonexistent_flow(self, client, app):
        ...
        response = await client.post(
            ...
            json={
                "node_type": "flow",
                "node_config": {"flow_id": "nonexistent_flow_12345"},
                "state": state
            }
        )
        assert response.status_code == 200
