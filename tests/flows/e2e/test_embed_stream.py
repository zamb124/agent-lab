...
async def embed_test_auth(frontend_container):
    ...
    Возвращает headers, flows_container, company_id и flow_id для cleanup.
    """
    ...
    # Создаем уникальный flow_id для изоляции тестов
    test_agent_id = f"test_agent_{company_id}"
    
    agent = FlowConfig(
        flow_id=test_agent_id,
        name="Test Agent",
        entry="main",
        nodes={"main": {"type": "llm_node", "prompt": "Test", "llm": {"model": "mock"}}},
    ...
    yield {"Authorization": f"Bearer {token}"}, flows_container, company_id, test_agent_id
    ...
    await flows_container.flow_repository.delete(test_agent_id)
    clear_context()
...
async def test_embed_settings_endpoint(
    ...
    auth_headers, flows_container, company_id, test_agent_id = embed_test_auth
    ...
    settings_response = await flows_client.get(
        f"/flows/api/v1/embed/{embed_id}/settings"
    )
...
@pytest.mark.asyncio
async def test_embed_settings_nonexistent_widget(flows_client: AsyncClient):
    """Тест что несуществующий виджет возвращает 404"""
    
    response = await flows_client.get(
        "/flows/api/v1/embed/nonexistent_embed_id_12345/settings"
    )
...
@pytest.mark.asyncio
async def test_embed_stream_basic_flow(flows_client: AsyncClient, frontend_client: AsyncClient, embed_test_auth):
    """
    ...
    stream_response = await flows_client.get(
        f"/flows/api/v1/embed/{embed_id}/stream",
...
@pytest.mark.asyncio
async def test_embed_stream_usage_count(flows_client: AsyncClient, frontend_client: AsyncClient, embed_test_auth):
    ...
    await flows_client.delete(
        f"/flows/api/v1/flows/usage_test_agent",
        headers=auth_headers
    )


@pytest.mark.asyncio
async def test_embed_stream_disabled_widget(flows_client: AsyncClient, frontend_client: AsyncClient, embed_test_auth):
    ...
    await flows_client.delete(
        f"/flows/api/v1/flows/disabled_test_agent",
        headers=auth_headers
    )


@pytest.mark.asyncio
async def test_embed_stream_with_context_id(flows_client: AsyncClient, frontend_client: AsyncClient, embed_test_auth):
    ...
    await flows_client.delete(
        f"/flows/api/v1/flows/context_test_agent",
        headers=auth_headers
    )


@pytest.mark.asyncio
async def test_embed_stream_nonexistent_widget(flows_client: AsyncClient):
    """Тест с несуществующим виджетом"""
    
    response = await flows_client.get(
        "/flows/api/v1/embed/nonexistent_widget_123/stream",
        params={"message": "Test"}
    )
