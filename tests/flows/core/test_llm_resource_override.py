"""Слияние NodeLLMOverride с LLM-ресурсом по llm_resource_key."""
...
async def test_resolve_merges_inline_llm_resource() -> None:
    ...
    ov = NodeLLMOverride(llm_resource_key="gpt", temperature=0.9)
    merged = await resolve_llm_override_with_resource_key(
        llm_override=ov,
...
async def test_resolve_shared_resource_from_repository() -> None:
    ...
    ov = NodeLLMOverride(llm_resource_key="shared", model="should-not-apply")
    merged = await resolve_llm_override_with_resource_key(
        llm_override=ov,
        flow_resources=flow_resources,
        skill_resources=None,
