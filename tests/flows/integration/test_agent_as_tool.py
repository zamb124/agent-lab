            ...
            response_queue=[
                {
                    "type": "tool_call",
                    "tool": "support_agent",
                    "args": {"query": "Помоги пользователю с заказом"},
                },
                {
                    "type": "tool_call",
                    "tool": "ask_user",
                    "args": {"question": "Какой у вас номер заказа?"},
                },
            ]
        )
...
class TestSubagentMessagesIntegrity:
    ...
    async def test_full_resume_cycle(self, main_flow_with_subagent: Dict[str, Any], app):
        ...
        assert result2.response, f"Должен быть финальный ответ, state: {result2}"
