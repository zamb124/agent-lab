...
async def test_list_by_space_and_list_for_user_with_space_filter(
    ...
    await channel_repo.upsert_member("ch_1", "alice", "member", company_id=company_id)
    await channel_repo.upsert_member("ch_2", "alice", "member", company_id=company_id)
    ...
    assert {c.channel_id for c in for_user} == {"ch_1", "ch_2"}
