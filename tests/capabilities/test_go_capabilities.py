...
@pytest.mark.asyncio
async def test_go_documentation_describes_every_capability(sandbox_services) -> None:
    await assert_language_documentation(LANGUAGE, sandbox_services)


@pytest.mark.asyncio
async def test_go_sdk_contains_every_manifest_capability(
    sandbox_services,
    flows_client_http,
    auth_headers_system,
) -> None:
...
@pytest.mark.asyncio
async def test_go_runs_every_static_platform_capability(
    sandbox_services,
    flows_client_http,
    auth_headers_system,
    unique_id,
...
@pytest.mark.asyncio
async def test_go_runs_interrupt_platform_capability(sandbox_services) -> None:
    await assert_language_runs_interrupt_platform_capabilities(LANGUAGE, sandbox_services)


@pytest.mark.asyncio
async def test_go_calls_tools_written_in_every_language(
    flows_client_http,
    auth_headers_system,
