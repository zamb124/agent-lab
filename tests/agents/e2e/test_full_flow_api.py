...
""",
    ...
    async def test_create_simple_flow_via_api(self, client, auth_headers_system):
        ...
        data = response.json()
        assert data["agent_id"] == "e2e_simple_flow"
...
class TestE2EInterruptInCodeNode:
    ...
    async def test_create_flow_with_interrupt_function(self, client, auth_headers_system):
        ...
        response = await client.post(
            "/agents/api/v1/agents/",
            json={
                "agent_id": "e2e_interrupt_function_flow",
...
class TestE2EInterruptInCodeNodeV2:
    ...
    async def test_interrupt_resume_full_cycle(self, client):
        ...
        """
        import uuid
        agent_id = f"e2e_interrupt_v2_{uuid.uuid4().hex[:8]}"
        context_id = uuid.uuid4().hex[:8]
        session_id = f"{agent_id}:session_{context_id}"

        # 1. Создаём flow с AgentInterrupt в function ноде
...
def run(state):
    ...
    # Бросаем AgentInterrupt для запроса имени
...
class TestE2EExternalAPIWithVarAuth:
    ...
    async def test_create_flow_with_external_api_var_auth(self, client, auth_headers_system):
        ...
        response = await client.post(
            "/agents/api/v1/agents/",
            json={
                "agent_id": "e2e_external_api_var_flow",
...
class TestE2EMultipleInterruptScenarios:
    ...
    async def setup_complex_flow(self, client, auth_headers_system):
        ...
        response = await client.post(
            "/agents/api/v1/agents/",
            json={
                "agent_id": "e2e_complex_interrupt_flow",
...
""",
    ...
    async def test_create_complex_flow(self, client, auth_headers_system):
        ...
        response = await client.get("/agents/api/v1/e2e_complex_interrupt_flow", headers=auth_headers_system)
        assert response.status_code == 200
...
class TestE2EExternalAgentInAgent:
    """E2E: Внешний A2A агент в flow."""

    @pytest.mark.asyncio
    async def test_create_flow_with_remote_agent_node(self, client, auth_headers_system):
        """Создание flow с remote_agent нодой через API."""
        # Проверяем что конфиг с remote_agent принимается
        response = await client.post(
            "/agents/api/v1/agents/",
            json={
                "agent_id": "e2e_remote_agent_flow",
                "name": "E2E Remote Agent Agent",
                "entry": "call_agent",
                "nodes": {
                    "call_agent": {
                        "type": "remote_agent",
...
""",
                ...
                "edges": [
                    {"from": "call_agent", "to": "process_result"},
        ...
        assert response.status_code == 200
        data = response.json()
        assert data["agent_id"] == "e2e_remote_agent_flow"
...
""",
        ...
        create_resp = await client.post(
            "/agents/api/v1/agents/",
            json={
                "agent_id": "e2e_full_scenario_flow",
