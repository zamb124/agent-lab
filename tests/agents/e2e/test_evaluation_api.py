...
async def send_a2a_message(
    ...
    if session_id is None:
        session_id = f"{agent_id}:test-{uuid_lib.uuid4()}"
...
class TestEvaluationDBStorage:
    ...
    async def test_result_saved_to_db(self, client, container, mock_llm):
        ...
        assert found.agent_id == "example_react"
...
class TestEvaluationResultsAPI:
    ...
    async def test_get_results_returns_list(self, client, container, mock_llm):
        ...
        response = await client.get(
            "/agents/api/v1/evaluation/results",
            params={"agent_id": "example_react", "skill_id": "test_full"},
        )
    ...
    async def test_get_results_with_limit(self, client, container, mock_llm):
        ...
        response = await client.get(
            "/agents/api/v1/evaluation/results",
            params={"agent_id": "example_react", "skill_id": "test_full", "limit": 2},
        )
    ...
    async def test_get_results_empty_for_unknown_flow(self, client):
        ...
        response = await client.get(
            "/agents/api/v1/evaluation/results",
            params={"agent_id": "nonexistent_flow_12345", "skill_id": "default"},
        )
    ...
    async def test_get_summary_structure(self, client, container, mock_llm):
        ...
        response = await client.get(
            "/agents/api/v1/evaluation/results/summary",
            params={"agent_id": "example_react", "skill_id": "test_full"},
        )
    ...
    async def test_get_specific_test_result_found(self, client, container, mock_llm):
        ...
        response = await client.get(
            f"/agents/api/v1/evaluation/results/{test_case_id}",
            params={"agent_id": "example_react", "skill_id": "test_full"},
        )
    ...
    async def test_get_specific_test_result_not_found(self, client):
        ...
        response = await client.get(
            "/agents/api/v1/evaluation/results/nonexistent_test_12345",
            params={"agent_id": "example_react", "skill_id": "test_full"},
        )
    ...
    async def test_delete_old_results(self, client, container, mock_llm):
        ...
        response = await client.delete("/agents/api/v1/evaluation/results", params={"days": 1})
    ...
    async def test_get_results_with_date_filter(self, client, container, mock_llm):
        ...
        response = await client.get(
            "/agents/api/v1/evaluation/results",
            params={"agent_id": "example_react", "skill_id": "test_full", "run_date": today},
        )
    ...
    async def test_get_results_required_params(self, client):
        """GET /api/v1/evaluation/results требует agent_id."""
        response = await client.get("/agents/api/v1/evaluation/results")

        # Должна быть ошибка валидации - agent_id обязателен
        assert response.status_code == 422

    async def test_summary_required_params(self, client):
        """GET /api/v1/evaluation/results/summary требует agent_id."""
        response = await client.get("/agents/api/v1/evaluation/results/summary")
...
class TestFlowsAPI:
    ...
    async def test_get_flow_with_evaluation(self, client):
        """GET /api/v1/agents/{agent_id} возвращает flow с evaluation."""
        response = await client.get("/agents/api/v1/agents/example_react")
    ...
    async def test_get_flow_without_evaluation(self, client):
        """GET /api/v1/agents/{agent_id} для flow без evaluation."""
    ...
    async def test_get_flow_not_found(self, client):
        """GET /api/v1/agents/{agent_id} для несуществующего flow."""
        response = await client.get("/agents/api/v1/nonexistent_flow_12345")
...
class TestAgentTypeEvaluation:
    """Тесты для type=agent - агент-тестер + агент-судья.

    Используем mock_llm_redis потому что invoke_llm.kiq() выполняется в worker.
    """

    async def test_agent_test_runs_with_tester_and_judge(self, client, taskiq_worker, mock_llm_redis, mock_llm):
        """Проверяем что agent тест запускает тестера и судью."""
        # mock_llm_redis для invoke_llm (тестер, судья)
        # mock_llm для process_agent_task (flow)
        ...
        data = await run_evaluation(client, "example_react", "agent_tester_judge", "test_full")

        state = get_task_state(data)
        assert state in ("completed", "failed"), f"Agent test не завершился: {state}"

    async def test_agent_test_dialog_saved(self, client, taskiq_worker, mock_llm_redis, mock_llm, container):
        ...
        await run_evaluation(client, "example_react", "agent_test", "test_full")
        ...
        result = results[0]
        assert result.test_case_id == "agent_test"
        assert len(result.dialog) > 0

    async def test_agent_test_scores_saved(self, client, mock_llm_redis, mock_llm, container):
    ...
    async def test_agent_test_judge_fail_marks_test_failed(self, client, mock_llm_redis, mock_llm):
    ...
    async def test_agent_test_max_turns_limit(self, client, mock_llm_redis, mock_llm):
    ...
    async def test_agent_test_graph_flow(self, client, mock_llm_redis, mock_llm):
        """Проверяем agent тест на графовом flow."""
        await setup_mock_llm_redis(
            mock_llm_redis,
            [
                "Хочу сделать заказ",
        ...
        state = get_task_state(data)
        assert state in ("completed", "failed"), f"Graph agent test не завершился: {state}"

    async def test_agent_test_loop_detection(self, client, mock_llm_redis, mock_llm, container):
