...
class TestResolveMockConfig:
    ...
    def test_flow_overrides_global(self):
        ...
        result = resolve_mock_config(global_mock=global_mock, agent_mock=agent_mock)
    ...
    def test_skill_overrides_flow(self):
        ...
        result = resolve_mock_config(agent_mock=agent_mock, skill_mock=skill_mock)
    ...
    def test_request_overrides_all(self):
        ...
        agent_mock = {"enabled": True, "tools": {"t1": "f"}}
    ...
    def test_agents_merge(self):
        """Agents мержатся между уровнями."""
        global_mock = {"agents": {"agent1": "g1"}}
        agent_mock = {"agents": {"agent2": "f2"}}
        skill_mock = {"agents": {"agent1": "s1", "agent3": "s3"}}
        ...
        assert result.agents["agent1"] == "s1"  # skill переопределил
        assert result.agents["agent2"] == "f2"  # от flow
        assert result.agents["agent3"] == "s3"  # от skill

    def test_nodes_merge(self):
        """Nodes мержатся между уровнями."""
        global_mock = {"nodes": {"node1": {"key1": "g1"}}}
        agent_mock = {"nodes": {"node2": {"key2": "f2"}}}
    ...
    def test_permission_groups_full_replace(self):
        ...
        agent_mock = {"permission_groups": ["testers", "qa"]}
    ...
    def test_none_llm_does_not_replace(self):
        ...
        agent_mock = {"llm": None}  # Явный None
...
class TestGetMockForAgent:
    ...
    def test_returns_mock_when_exists(self):
        ...
        assert get_mock_for_agent(state, "consultant") == "Mock консультация"
        assert get_mock_for_agent(state, "analyzer") == {"result": "analysis"}
    ...
    def test_returns_none_when_not_exists(self):
        ...
        state = ExecutionState(
            ...
            mock={
                "enabled": True,
                "agents": {"consultant": "mock"}
            }
        )
        
        assert get_mock_for_agent(state, "unknown_agent") is None
    ...
    def test_returns_none_when_mock_disabled(self):
        ...
        assert get_mock_for_agent(state, "consultant") is None

    def test_returns_none_when_no_mock_config(self):
        """Возвращает None когда нет __mock__ в state."""
        assert get_mock_for_agent({}, "consultant") is None

    def test_returns_none_when_state_is_none(self):
        """Возвращает None когда state is None."""
        assert get_mock_for_agent(None, "consultant") is None
...
class TestCheckMockPermission:
    ...
    def test_admin_has_permission(self, monkeypatch):
        ...
        # Патчим в apps.agents.config (там используется свой _settings)
