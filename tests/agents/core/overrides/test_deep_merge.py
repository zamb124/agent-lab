...
class TestDeepMergeBasics:
    ...
    def test_empty_base_returns_override(self):
        ...
        assert result is not override  # Новый dict
    ...
    def test_empty_override_returns_base_copy(self):
        ...
        assert result == {"key": "value"}
        assert result is not base  # Новый dict
...
class TestDeepMergeNested:
    ...
    def test_nested_dict_merge(self):
        ...
        assert result["llm"]["model"] == "gpt-4o"  # Сохранен
        assert result["llm"]["temperature"] == 0.1  # Переопределен
    ...
    def test_deeply_nested_merge(self):
        ...
        assert result["level1"]["level2"]["level3"]["a"] == 1  # Сохранен
        assert result["level1"]["level2"]["level3"]["b"] == 99  # Переопределен
        assert result["level1"]["level2"]["level3"]["c"] == 3  # Добавлен
...
class TestDeepMergeExclude:
    ...
    def test_agent_id_excluded_by_default(self):
        """agent_id исключен по умолчанию."""
        base = {"name": "base"}
        override = {"agent_id": "some_agent", "name": "new"}
    ...
    def test_custom_exclude_set(self):
        ...
        assert result["a"] == 1  # Не перезаписан
        assert result["b"] == 2  # Не перезаписан
        assert result["c"] == 30  # Добавлен


class TestDeepMergeNodeConfig:
    """Тесты на реальных примерах NodeConfig."""

    def test_react_node_prompt_override(self):
        """Переопределение prompt в react_node."""
        base = {
            "type": "react_node",
        ...
        assert result["type"] == "react_node"  # Сохранен
        assert result["prompt"] == "Новый промпт"  # Переопределен
        assert result["tools"] == ["calculator", "ask_user"]  # Сохранен
        assert result["llm"]["model"] == "gpt-4o"  # Сохранен
        assert result["llm"]["temperature"] == 0.1  # Переопределен
        assert "node_id" not in result  # Исключен

    def test_react_node_tools_override(self):
    ...
    def test_function_node_code_override(self):
        ...
        override = {
            "code": "def run(s): s['modified'] = True; return s"
        }
    ...
    def test_input_mapping_override(self):
        ...
        assert result["input_mapping"]["content"] == "@state:user_query"  # Сохранен
        assert result["input_mapping"]["user_name"] == "@state:custom.name"  # Переопределен
        assert result["input_mapping"]["new_field"] == "@state:new"  # Добавлен
...
class TestDeepMergeToolReference:
    ...
    def test_inline_tool_code_override(self):
        ...
        assert "Hi" in result["code"]  # Переопределен
        assert result["description"] == "Форматирует приветствие"  # Сохранен
        # args_schema мержится как dict
        assert "name" in result["args_schema"]  # Сохранен
        assert "style" in result["args_schema"]  # Добавлен
...
class TestDeepMergeSkillConfig:
    ...
    def test_skill_variables_merge(self):
        ...
        skill_variables = {
            "max_length": 200,
            "skill_specific": "value"
        }

        result = deep_merge(base_variables, skill_variables)

        assert result["company_name"] == "BaseCompany"  # Сохранен
        assert result["max_length"] == 200  # Переопределен
        assert result["skill_specific"] == "value"  # Добавлен
    ...
    def test_skill_nodes_merge(self):
        ...
        # main мержится
        assert result["main"]["type"] == "react_node"  # Сохранен
        assert result["main"]["prompt"] == "Skill prompt"  # Переопределен
        assert result["main"]["tools"] == ["t1"]  # Сохранен
        assert result["main"]["llm"]["temperature"] == 0.1  # Добавлен
        # helper сохранен
