...
import pytest
from apps.agents.src.models import AgentConfig, Edge, SkillConfig
from apps.agents.src.mock.config import MockConfig
from apps.agents.src.mock.resolver import (
    resolve_mock_config,
    get_mock_for_tool,
    get_mock_for_agent,
    get_mock_for_node,
...
class TestMockConfigMerge:
    ...
    def test_empty_skill_mock_uses_agent_mock(self):
        ...
        config = resolve_mock_config(agent_mock=agent_mock, skill_mock=None)

        assert config.enabled is True
        assert config.tools["calculator"] == 42

    def test_skill_mock_overrides_agent_mock(self):
        ...
        config = resolve_mock_config(agent_mock=agent_mock, skill_mock=skill_mock)
    ...
    def test_skill_mock_merges_agents(self):
        ...
        assert config.agents["subagent1"] == "skill response 1"
        assert config.agents["subagent2"] == "agent response 2"
        assert config.agents["subagent3"] == "skill response 3"
...
class TestMockResolverFunctions:
    ...
    def test_get_agent_mock_from_state(self):
        ...
        state = ExecutionState(
            ...
            mock={
                "enabled": True,
                "agents": {"subagent": "mocked response"}
            }
        )

        result = get_mock_for_agent(state, "subagent")
    ...
    def test_disabled_mock_returns_none(self):
        ...
        assert get_mock_for_agent(state, "subagent") is None
...
class TestMockResolvePriority:
    ...
    def test_request_mock_highest_priority(self):
        ...
        agent_mock = {"enabled": True, "tools": {"t1": "flow"}}
    ...
    def test_agent_mock_over_global(self):
        """Flow mock над global mock."""
        global_mock = {"enabled": True, "tools": {"t1": "global"}}
        agent_mock = {"enabled": True, "tools": {"t1": "flow"}}

        config = resolve_mock_config(global_mock=global_mock, agent_mock=agent_mock)
...
class TestMockConfigAllLevels:
    ...
    def test_all_levels_mock_priority(self):
        ...
        assert config.agents["sub1"] == "skill"
        assert config.agents["sub2"] == "skill"
    ...
    def test_example_react_test_full_skill(self):
        ...
        agent_mock = {
            "enabled": False,
            "tools": {"calculator": 42},
            "agents": {"example_subagent": "Flow mock response"},
            "nodes": {
        ...
        skill_mock = {
            ...
            "agents": {"example_subagent": "Mock subagent response"},
            "nodes": {
                "main": {"response": "Mock node response"},
                "direct_subagent": {"response": "Mock direct subagent node response"}
            }
        }
        ...
        assert config.agents["example_subagent"] == "Mock subagent response"

        # Nodes merged with skill priority
        assert config.nodes["main"]["response"] == "Mock node response"
        assert config.nodes["direct_subagent"]["response"] == "Mock direct subagent node response"
