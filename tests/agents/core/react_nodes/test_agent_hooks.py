...
class AgentWithHooks(ReactNode):
...
class AgentWithoutHooks(ReactNode):
...
@pytest.fixture
def runner_with_hooks(agent_config):
    """ReactNodeRunner с агентом, у которого есть хуки."""
    agent = AgentWithHooks(node_config=agent_config)
...
@pytest.fixture
def runner_without_hooks(agent_config):
    """ReactNodeRunner с агентом без хуков."""
    agent = AgentWithoutHooks(node_config=agent_config)
    return ReactNodeRunner(
        node_config=agent_config,
        tools=[],
        llm=None,
...
class TestHooksWithoutAgent:
    ...
    async def test_default_hooks_return_unchanged(self, runner_without_hooks):
        ...
        agent = runner_without_hooks.react_node
