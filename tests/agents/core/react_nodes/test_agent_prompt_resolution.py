...
@pytest.fixture
def runner(agent_config):
    """ReactNodeRunner для тестирования."""
...
class TestSystemVariablesInPrompts:
    ...
    @pytest.mark.asyncio
    async def test_current_date_in_prompt(self, runner, agent_config):
        """Проверяет, что {current_date} резолвится в промпте."""
        agent_config.prompt = "Сегодня {current_date}. Ты помощник."
        runner.prompt = agent_config.prompt
    ...
    @pytest.mark.asyncio
    async def test_current_time_in_prompt(self, runner, agent_config):
        """Проверяет, что {current_time} резолвится в промпте."""
        agent_config.prompt = "Текущее время: {current_time}"
        runner.prompt = agent_config.prompt
    ...
    @pytest.mark.asyncio
    async def test_user_id_in_prompt(self, runner, agent_config):
        """Проверяет, что {user_id} резолвится в промпте."""
        agent_config.prompt = "Пользователь: {user_id}. Ты помощник."
        runner.prompt = agent_config.prompt
    ...
    @pytest.mark.asyncio
    async def test_user_name_in_prompt(self, runner, agent_config):
        """Проверяет, что {user_name} резолвится в промпте."""
        agent_config.prompt = "Привет, {user_name}! Ты помощник."
        runner.prompt = agent_config.prompt
    ...
    @pytest.mark.asyncio
    async def test_user_email_in_prompt(self, runner, agent_config):
        """Проверяет, что {user_email} резолвится в промпте."""
        agent_config.prompt = "Email пользователя: {user_email}"
        runner.prompt = agent_config.prompt
    ...
    @pytest.mark.asyncio
    async def test_all_system_variables_in_prompt(self, runner, agent_config):
        """Проверяет все системные переменные в одном промпте."""
        agent_config.prompt = """Привет, {user_name}!
...
class TestFlowVariablesInPrompts:
    ...
    @pytest.mark.asyncio
    async def test_flow_variable_in_prompt(self, runner, agent_config):
        """Проверяет, что переменная flow резолвится в промпте."""
        agent_config.prompt = "Компания: {company_name}. Ты помощник."
        runner.prompt = agent_config.prompt
    ...
    @pytest.mark.asyncio
    async def test_multiple_flow_variables_in_prompt(self, runner, agent_config):
        """Проверяет несколько переменных flow в промпте."""
        agent_config.prompt = "Компания: {company_name}, Телефон: {phone}"
        runner.prompt = agent_config.prompt
    ...
    @pytest.mark.asyncio
    async def test_flow_variable_with_default(self, runner, agent_config):
        """Проверяет переменную flow с default значением."""
        agent_config.prompt = "Город: {city|не указан}"
        runner.prompt = agent_config.prompt
...
class TestLocalAgentVariablesInPrompts:
    ...
    @pytest.mark.asyncio
    async def test_local_variable_in_prompt(self, runner, agent_config):
        """Проверяет, что локальная переменная агента резолвится в промпте."""
        agent_config.prompt = "Роль: {role}. Ты помощник."
        agent_config.local_variables = {"role": "консультант"}
        runner.prompt = agent_config.prompt
    ...
    @pytest.mark.asyncio
    async def test_local_variable_overrides_flow_variable(self, runner, agent_config):
        """Проверяет, что локальная переменная имеет приоритет над flow переменной."""
        agent_config.prompt = "Роль: {role}"
        agent_config.local_variables = {"role": "локальная_роль"}
        runner.prompt = agent_config.prompt
...
class TestCombinedVariablesInPrompts:
    ...
    @pytest.mark.asyncio
    async def test_system_and_flow_variables(self, runner, agent_config):
...
Сегодня: {current_date}"""
    ...
    @pytest.mark.asyncio
    async def test_system_flow_and_local_variables(self, runner, agent_config):
...
class TestPromptSyntaxInAgents:
    ...
    @pytest.mark.asyncio
    async def test_optional_variable_with_default(self, runner, agent_config):
        """Проверяет опциональную переменную с default в промпте агента."""
        agent_config.prompt = "Email: {?support_email|support@example.com}"
        runner.prompt = agent_config.prompt
    ...
    @pytest.mark.asyncio
    async def test_conditional_block_in_prompt(self, runner, agent_config):
        """Проверяет условный блок в промпте агента."""
        agent_config.prompt = """Ты помощник.
...
- Инструкция 2
    ...
    @pytest.mark.asyncio
    async def test_nested_variables_in_prompt(self, runner, agent_config):
        """Проверяет вложенные переменные в промпте агента."""
        agent_config.prompt = "Город: {city.name|не указан}, Улица: {city.street|не указана}"
        runner.prompt = agent_config.prompt
    ...
    @pytest.mark.asyncio
    async def test_complex_prompt_with_all_syntax(self, runner, agent_config):
...
Сегодня: {current_date}"""
        agent_config.local_variables = {"is_vip": True}
        runner.prompt = agent_config.prompt
...
class TestPromptResolutionWithoutContext:
    ...
    @pytest.mark.asyncio
    async def test_system_variables_without_user_context(self, runner, agent_config):
        """Проверяет, что системные переменные работают без контекста пользователя."""
        clear_context()
        
        agent_config.prompt = "Сегодня {current_date}, время {current_time}"
        runner.prompt = agent_config.prompt
    ...
    @pytest.mark.asyncio
    async def test_user_variables_without_context(self, runner, agent_config):
        """Проверяет, что переменные пользователя не резолвятся без контекста."""
        clear_context()
        
        agent_config.prompt = "Пользователь: {user_name|неизвестен}"
        runner.prompt = agent_config.prompt
...
class TestPromptResolutionEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_empty_prompt(self, runner, agent_config):
    ...
    @pytest.mark.asyncio
    async def test_prompt_without_variables(self, runner, agent_config):
        """Проверяет промпт без переменных."""
        agent_config.prompt = "Ты помощник. Отвечай на вопросы."
        runner.prompt = agent_config.prompt
    ...
    @pytest.mark.asyncio
    async def test_prompt_with_unknown_variable_safe_mode(self, runner, agent_config):
        """Проверяет промпт с неизвестной переменной в safe режиме."""
        agent_config.prompt = "Привет, {unknown_var}!"
        runner.prompt = agent_config.prompt
    ...
    @pytest.mark.asyncio
    async def test_prompt_with_escaped_braces(self, runner, agent_config):
        """Проверяет промпт с экранированными скобками."""
        # В Python строке \\{ становится \{ после обработки
        agent_config.prompt = "Путь: \\{config_path\\} или {config_path|C:\\\\Windows}"
        runner.prompt = agent_config.prompt
...
class TestVariablesFromMetadata:
    ...
    @pytest.mark.asyncio
    async def test_metadata_variables_override_flow_variables(self, runner, agent_config):
    ...
    @pytest.mark.asyncio
    async def test_metadata_variables_partial_override(self, runner, agent_config):
        """Проверяет частичное переопределение variables из metadata."""
        agent_config.prompt = "Компания: {company_name}, Телефон: {phone}, Email: {email}"
        runner.prompt = agent_config.prompt
...
class TestJsonDictVariablesInPrompts:
    ...
    @pytest.mark.asyncio
    async def test_dict_variable_dot_notation(self, runner, agent_config):
        """Проверяет доступ к dict переменной через точку."""
        agent_config.prompt = "Пользователь: {user.name}, Email: {user.email}"
        runner.prompt = agent_config.prompt
    ...
    @pytest.mark.asyncio
    async def test_deeply_nested_dict_variable(self, runner, agent_config):
    ...
    @pytest.mark.asyncio
    async def test_dict_variable_with_default(self, runner, agent_config):
        """Проверяет dict переменную с default значением."""
        agent_config.prompt = "Город: {city.name|не указан}, Район: {city.district|не указан}"
        runner.prompt = agent_config.prompt
    ...
    @pytest.mark.asyncio
    async def test_dict_variable_optional(self, runner, agent_config):
        """Проверяет опциональную dict переменную."""
        agent_config.prompt = "Пользователь: {?user.name|Гость}, Email: {?user.email}"
        runner.prompt = agent_config.prompt
    ...
    @pytest.mark.asyncio
    async def test_complex_json_structure(self, runner, agent_config):
        """Проверяет сложную JSON структуру с множественными уровнями вложенности."""
        agent_config.prompt = """Конфигурация:
...
Пользователь БД: {config.database.user.name}"""
    ...
    @pytest.mark.asyncio
    async def test_dict_variable_with_list_access(self, runner, agent_config):
        """Проверяет доступ к элементам списка внутри dict."""
        agent_config.prompt = "Первый контакт: {contacts.0.name}, Телефон: {contacts.0.phone}"
        runner.prompt = agent_config.prompt
    ...
    @pytest.mark.asyncio
    async def test_dict_variable_mixed_with_simple(self, runner, agent_config):
    ...
    @pytest.mark.asyncio
    async def test_dict_variable_from_metadata(self, runner, agent_config):
        """Проверяет dict переменную, переопределенную из metadata."""
        agent_config.prompt = "Пользователь: {user.name}, Роль: {user.role}"
        runner.prompt = agent_config.prompt
