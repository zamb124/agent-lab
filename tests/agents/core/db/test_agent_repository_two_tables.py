"""
СТРОГИЕ тесты AgentRepository с разделением на две таблицы.
...
class TestAgentRepositoryTwoTables:
    """СТРОГИЕ тесты AgentRepository с двумя таблицами."""
    ...
    async def test_set_saves_to_both_tables(self, app, unique_id):
        ...
        agent_id = f"test_two_tables_set_{unique_id}"
        agent = AgentConfig(
            agent_id=agent_id,
            name="Test Agent",
    ...
    async def test_get_latest_reads_from_agents_table(self, app, unique_id):
        ...
        agent_id = f"test_get_latest_source_{unique_id}"
        agent = AgentConfig(
            agent_id=agent_id,
            name="Test Agent",
            entry="main",
        ...
        loaded = await repo.get_latest(agent_id)
    ...
    async def test_get_version_reads_from_agents_versions_table(self, app, unique_id):
        ...
        agent_id = f"test_get_version_source_{unique_id}"
        agent = AgentConfig(
            agent_id=agent_id,
            name="Test Agent",
            entry="main",
        ...
        loaded_agent = await repo.get(agent_id)
        ...
        loaded = await repo.get_version(agent_id, version)
    ...
    async def test_list_all_reads_from_agents_table_only(self, app, unique_id):
        ...
        agent_id = f"test_list_all_isolation_{unique_id}"
        agent = AgentConfig(
            agent_id=agent_id,
            name="Test Agent",
            entry="main",
        ...
        retrieved_agent = await repo.get(agent_id)
        ...
        assert agent_id in agent_ids, "list_all() ДОЛЖЕН вернуть агента независимо от количества версий"
        assert agent_ids.count(agent_id) == 1, "list_all() ДОЛЖЕН вернуть каждого агента только один раз"
    ...
    async def test_list_all_performance_with_many_versions(self, app, unique_id):
        ...
        agent = AgentConfig(
            agent_id=many_versions_agent_id,
    ...
    async def test_delete_removes_from_both_tables(self, app, unique_id):
        ...
        agent_id = f"test_delete_both_tables_{unique_id}"
        agent = AgentConfig(
            agent_id=agent_id,
            name="Test Agent",
            entry="main",
        ...
        deleted_agent = await repo.get(agent_id)
        assert deleted_agent is None, "Агент ДОЛЖЕН быть удалён"

        # Проверяем что версии удалены
        remaining_versions = await repo.list_versions(agent_id)
    ...
    async def test_rollback_copies_from_agents_versions_to_agents(self, app, unique_id):
        ...
        agent_id = f"test_rollback_copy_{unique_id}"
        agent = AgentConfig(
            agent_id=agent_id,
            name="Test Agent v1",
            entry="main",
        ...
        version2_data = await repo.get_version(agent_id, version2)
    ...
    async def test_multiple_agents_independence(self, app, unique_id):
        ...
        agent_ids = []
        for i in range(10):
            agent_id = f"test_independence_{unique_id}_{i}"
            agent_ids.append(agent_id)
            agent = AgentConfig(
                agent_id=agent_id,
                name=f"Test Agent {i}",
                entry="main",
                nodes={"main": {"type": "react_node", "prompt": f"Prompt {i}"}},
            )
            await repo.set(agent)
        ...
        for agent_id in agent_ids:
            ...
            assert agent is not None, f"Агент {agent_id} ДОЛЖЕН быть доступен"
        ...
        for agent_id in agent_ids:
            assert agent_id in all_agent_ids, f"Агент {agent_id} ДОЛЖЕН быть в списке"
    ...
    async def test_table_isolation_strict(self, app, unique_id):
        ...
        agent_id = f"test_table_isolation_{unique_id}"
        agent = AgentConfig(
            agent_id=agent_id,
            name="Isolation Test",
            entry="main",
        ...
        version_key = repo._get_key(f"{agent_id}_v{version}")
