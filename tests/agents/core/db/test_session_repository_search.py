...
from apps.agents.src.models import SessionConfig, SessionStatus
...
class TestStateRepositorySearchSessions:
    ...
    async def test_search_by_agent_id(self, app, unique_id):
        ...
        sessions, total = await repo.search_sessions(agent_id=target_flow, limit=100)

        assert total == 1
        assert sessions[0].session_id == session1_id
        assert sessions[0].agent_id == target_flow
    ...
    async def test_search_pagination(self, app, unique_id):
        ...
        for i in range(sessions_to_create):
            session_id = f"{agent_id}:session_{i}_{unique_id}"
        ...
        for i in range(sessions_to_create):
            await repo.delete(f"{agent_id}:session_{i}_{unique_id}")
    ...
    async def test_search_ordering(self, app, unique_id):
        ...
        session1_id = f"{agent_id}:session1_{unique_id}"
        session2_id = f"{agent_id}:session2_{unique_id}"
        session3_id = f"{agent_id}:session3_{unique_id}"
