...
class TestReactNodeLLMConfig:
    """Тесты передачи LLM конфига из ReactNode."""

    @pytest.mark.asyncio
    async def test_react_node_passes_llm_config_to_factory(self):
        """ReactNode передает llm config в get_llm."""
        from apps.agents.src.agent.nodes import ReactNode
        from apps.agents.src.models.node_config import NodeConfig, NodeLLMOverride, NodeType
        ...
        with patch("apps.agents.src.agent.nodes.get_llm") as mock_get_llm:
    ...
    @pytest.mark.asyncio
    async def test_react_node_passes_llm_config_dict(self):
        """ReactNode передает llm_config_dict в get_llm."""
        from apps.agents.src.agent.nodes import ReactNode
...
class TestLLMModelsServiceRealAPI:
    ...
    async def test_fetch_bothub_models_real_api(self):
        ...
        """
        from apps.agents.src.services.llm_models_service import LLMModelsService
        from apps.agents.src.db.llm_model_repository import LLMModelRepository
