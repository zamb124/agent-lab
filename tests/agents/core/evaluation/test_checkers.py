...
class TestPythonFunctionChecker:
    ...
    def test_function_checker_pass(self, runner):
        ...
        result = runner._execute_string_checker(
            "apps.agents.agents.example_react.checks.check_greeting", {}, "Привет! Рад вас видеть!"
        )
        assert result is True

    def test_function_checker_fail(self, runner):
        """Python функция - failed."""
        result = runner._execute_string_checker("apps.agents.agents.example_react.checks.check_greeting", {}, "Пока!")
        assert result is False

    def test_function_with_state(self, runner):
        """Python функция использует state."""
        result = runner._execute_string_checker(
            "apps.agents.agents.example_graph.checks.check_order_route", {"route": "order"}, "Заказ создан"
        )
        assert result is True
