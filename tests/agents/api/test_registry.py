...
class TestRegistryAgents:
    """Тесты /registry/agents endpoint."""

    @pytest.mark.asyncio
    async def test_get_agents_returns_list(self, client):
        """GET /registry/agents возвращает список flows как AgentCard[]."""
        response = await client.get("/agents/api/v1/registry/agents")
...
class TestRegistryTools:
    ...
    async def test_get_tools_returns_list(self, client):
        ...
        response = await client.get("/agents/api/v1/registry/tools")
...
class TestFlowSchema:
    """Тесты /registry/agents/{agent_id}/schema endpoint."""

    @pytest.mark.asyncio
    async def test_schema_returns_html(self, client):
        """GET /registry/agents/{agent_id}/schema возвращает HTML."""
        response = await client.get("/agents/api/v1/registry/agents/example_graph/schema")

        assert response.status_code == 200
        assert "text/html" in response.headers["content-type"]

    @pytest.mark.asyncio
    async def test_schema_contains_agent_name(self, client):
    ...
    async def test_schema_contains_nodes(self, client):
        ...
        # Процессоры - react_node ноды, показываются по имени агента
        # Проверяем что есть какие-то агенты (имена могут быть разные)
        assert "Агент" in html or "agent" in html.lower()
    ...
    async def test_schema_404_for_unknown_flow(self, client):
        ...
        response = await client.get("/agents/api/v1/registry/agents/nonexistent_flow/schema")
    ...
    @pytest.mark.asyncio
    async def test_react_node_schema_has_tools(self, client):
        """Схема react агента содержит tools."""
        response = await client.get("/agents/api/v1/registry/agents/example_react/schema")
        html = response.text
    ...
    @pytest.mark.asyncio
    async def test_react_node_schema_has_subagents(self, client):
        """Схема react агента содержит субагенты."""
        response = await client.get("/agents/api/v1/registry/agents/example_react/schema")
        html = response.text

        # example_react main агент имеет субагент example_subagent
        # Проверяем что субагент или его имя есть в схеме
        assert "subagent" in html.lower() or "Субагент" in html
    ...
    @pytest.mark.asyncio
    async def test_schema_agents_have_names(self, client):
        """Агенты показываются с именами, не node_id."""
        response = await client.get("/agents/api/v1/registry/agents/example_react/schema")
        html = response.text

        # example_main_agent имеет name "Главный агент"
        # Должно быть имя, а не "main"
        # Проверяем что есть какое-то осмысленное имя
