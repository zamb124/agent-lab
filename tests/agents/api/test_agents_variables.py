"""
Тесты для полей variables в API agents.
...
class TestAgentVariables:
    """Тесты на полную структуру variables в API agents"""
    ...
    async def test_create_agent_with_full_variables(self, client, app, unique_id):
        ...
        agent_id = f"test_vars_{unique_id}"
    ...
    async def test_get_agent_returns_full_variables(self, client, app, unique_id):
        ...
        agent_id = f"test_get_vars_{unique_id}"
        ...
        await client.post(
            ...
            json={
                ...
                "entry": "main",
                "nodes": {
                    "main": {"type": "react_node", "prompt": "Test", "tools": []}
                },
                "edges": [{"from": "main", "to": None}],
                "variables": {
                    "api_key": {
        ...
        response = await client.get(f"/agents/api/v1/agents/{agent_id}")
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_update_agent_preserves_variables(self, client, app, unique_id):
        ...
        agent_id = f"test_upd_vars_{unique_id}"
        ...
        response = await client.put(
            ...
            json={
                ...
                "entry": "main",
                "nodes": {
                    "main": {"type": "react_node", "prompt": "Updated", "tools": []}
                },
                "edges": [{"from": "main", "to": None}],
                "variables": {
                    "setting": {
    ...
    async def test_list_agents_returns_full_variables(self, client, app, unique_id):
        """GET /agents/ возвращает полную структуру variables для каждого агента"""
        agent_id = f"test_list_vars_{unique_id}"
        ...
        # Получаем список
        response = await client.get("/agents/api/v1/agents/")
        assert response.status_code == 200
    ...
    async def test_skill_variables_full_structure(self, client, app, unique_id):
        ...
        agent_id = f"test_skill_vars_{unique_id}"
        ...
        response = await client.post(
            ...
            json={
                ...
                "entry": "main",
                "nodes": {
                    "main": {"type": "react_node", "prompt": "@var:role", "tools": []}
                },
                "edges": [{"from": "main", "to": None}],
                "variables": {
                    "role": {
    ...
    async def test_skill_variables_simple_value_normalized(self, client, app, unique_id):
        ...
        agent_id = f"test_skill_simple_{unique_id}"
    ...
    async def test_variable_value_can_be_json_object(self, client, app, unique_id):
        ...
        agent_id = f"test_json_value_{unique_id}"
        ...
        get_response = await client.get(f"/agents/api/v1/agents/{agent_id}")
...
class TestSkillsAPIVariables:
    """Тесты на variables в отдельных ручках skills (/agents/{id}/skills)"""

    @pytest.mark.asyncio
    async def test_get_skill_returns_full_variables(self, client, app, unique_id):
        """GET /agents/{id}/skills/{skill_id} возвращает полную структуру variables"""
        agent_id = f"test_skill_api_{unique_id}"
        ...
        response = await client.get(f"/agents/api/v1/{agent_id}/skills/my_skill")
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_create_skill_with_full_variables(self, client, app, unique_id):
        """POST /agents/{id}/skills создаёт skill с полными variables"""
        agent_id = f"test_create_skill_{unique_id}"
        ...
        get_response = await client.get(f"/agents/api/v1/{agent_id}/skills/new_skill")
        assert get_response.status_code == 200
    ...
    async def test_update_skill_preserves_variables(self, client, app, unique_id):
        """PUT /agents/{id}/skills/{skill_id} сохраняет все поля variables"""
        agent_id = f"test_update_skill_{unique_id}"
        ...
        # Обновляем skill
        response = await client.put(
            f"/agents/api/v1/{agent_id}/skills/updatable",
            json={
                "name": "Updated Skill",
                "skill_body": {
                    "variables": {
        ...
        get_response = await client.get(f"/agents/api/v1/{agent_id}/skills/updatable")
        assert get_response.status_code == 200
        data = get_response.json()
