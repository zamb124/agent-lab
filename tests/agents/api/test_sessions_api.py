...
def create_test_state(
    ...
    Args:
        session_id: ID сессии (формат 'agent_id:context_id')
...
class TestSessionsAPI:
    ...
    async def test_list_sessions(self, client, app, unique_id):
        ...
        agent_id = f"flow_{unique_id}"
        user_id = f"user_{unique_id}"
        session_id = f"{agent_id}:context_{unique_id}"
        ...
        response = await client.get("/agents/api/v1/sessions/")
    ...
    async def test_list_sessions_with_user_filter(self, client, app, unique_id):
        ...
        state1 = create_test_state(
            ...
            user_id=target_user,
            agent_id=f"flow1_{unique_id}",
        )
        
        state2 = create_test_state(
            session_id=session2_id,
            user_id=other_user,
            agent_id=f"flow2_{unique_id}",
        )

        await repo.set(session1_id, state1)
        await repo.set(session2_id, state2)

        response = await client.get(f"/agents/api/v1/sessions/?user_id={target_user}")
    ...
    async def test_list_sessions_with_flow_filter(self, client, app, unique_id):
        """GET /api/v1/sessions с фильтром по agent_id."""
        container = get_container()
        repo = container.state_repository
        ...
        response = await client.get(f"/agents/api/v1/sessions/?agent_id={target_flow}")

        assert response.status_code == 200
        data = response.json()
        assert data["total"] == 1
        assert len(data["sessions"]) == 1
        assert data["sessions"][0]["agent_id"] == target_flow
    ...
    async def test_list_sessions_with_date_range(self, client, app, unique_id):
        ...
        from apps.agents.src.db.models import States
        ...
        response = await client.get(
            f"/agents/api/v1/sessions/?user_id={base_user}&date_from={date_from}&date_to={date_to}"
        )
    ...
    async def test_list_sessions_with_combined_filters(self, client, app, unique_id):
        ...
        state2 = create_test_state(
            session_id=session2_id,
            user_id=target_user,
            agent_id=f"other_flow_{unique_id}",
        )
        ...
        response = await client.get(
            f"/agents/api/v1/sessions/?user_id={target_user}&agent_id={target_flow}"
        )
    ...
    async def test_list_sessions_pagination(self, client, app, unique_id):
        ...
        for i in range(5):
            session_id = f"{agent_id}:context_{i}_{unique_id}"
            state = create_test_state(
                session_id=session_id,
                user_id=base_user,
                agent_id=agent_id,
        ...
        response1 = await client.get(
            f"/agents/api/v1/sessions/?user_id={base_user}&limit=2&offset=0"
        )
        response2 = await client.get(
            f"/agents/api/v1/sessions/?user_id={base_user}&limit=2&offset=2"
        )
    ...
    async def test_list_sessions_empty_result(self, client, app, unique_id):
        ...
        response = await client.get(
            f"/agents/api/v1/sessions/?user_id=nonexistent_user_{unique_id}"
        )
    ...
    async def test_list_sessions_response_structure(self, client, app, unique_id):
        ...
        response = await client.get(f"/agents/api/v1/sessions/?user_id={user_id}")
    ...
    async def test_list_sessions_limit_validation(self, client, app):
        ...
        response = await client.get("/agents/api/v1/sessions/?limit=0")
        assert response.status_code == 422

        response = await client.get("/agents/api/v1/sessions/?limit=501")
        assert response.status_code == 422

        response = await client.get("/agents/api/v1/sessions/?limit=50")
        assert response.status_code == 200

    @pytest.mark.asyncio
    async def test_list_sessions_offset_validation(self, client, app):
        """Проверка валидации параметра offset."""
        response = await client.get("/agents/api/v1/sessions/?offset=-1")
        assert response.status_code == 422

        response = await client.get("/agents/api/v1/sessions/?offset=0")
        assert response.status_code == 200
