"""
Интеграционные тесты для эндпоинта /agents/api/auth/me на agents сервисе.
...
class TestAuthMeAgents:
    """Интеграционные тесты /agents/api/auth/me для agents сервиса"""
    
    @pytest.mark.asyncio
    async def test_auth_me_success_agents(self, agents_client, auth_token):
    ...
    @pytest.mark.asyncio
    async def test_auth_me_no_token_agents(self, agents_client):
        """Запрос к agents без токена возвращает 401"""
        response = await agents_client.get("/agents/api/auth/me")
    ...
    @pytest.mark.asyncio
    async def test_auth_me_invalid_token_agents(self, agents_client):
    ...
    @pytest.mark.asyncio
    async def test_auth_me_expired_token_agents(self, agents_client):
    ...
    @pytest.mark.asyncio
    async def test_auth_me_response_structure_agents(self, agents_client, auth_token):
    ...
    async def test_auth_me_consistency_between_services(
        ...
        agents_response = await agents_client.get(
            "/agents/api/auth/me",
            cookies={"auth_token": auth_token}
        )
