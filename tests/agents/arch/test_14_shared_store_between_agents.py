...
@pytest.mark.asyncio
async def test_01_subagent_changes_store_coordinator_sees(migrated_db,  agent_factory, unique_id, agent_repo):
    """
...
async def call_subagent(state):
    ...
    # Вызываем субагента с тем же session_id что и у координатора
    session_id = state.get("session_id")
    if session_id:
        result = await subagent.ainvoke(state, config={"configurable": {"session_id": session_id}})
    else:
        # Если session_id нет в state, используем None (новый checkpoint)
        result = await subagent.ainvoke(state, config={"configurable": {"session_id": None}})
...
""",
    ...
    assert "Большие Каменщики" in messages_content, f"Координатор должен был увидеть 'Большие Каменщики' в сообщениях: {messages_content}"
    
    print("✅ Тест 1 пройден: Координатор видит изменения от субагента")
    print(f"   Store после работы субагента: {result['store']}")


@pytest.mark.asyncio
async def test_02_coordinator_sets_store_subagent_sees(migrated_db,  agent_factory, unique_id, agent_repo):
    """
...
""",
    ...
    print("✅ Тест 2 пройден: Субагент видит данные от координатора")
    print(f"   Store: {result['store']}")


@pytest.mark.asyncio
async def test_03_multiple_subagents_share_store(migrated_db,  agent_factory, unique_id, agent_repo):
    """
...
""",
    ...
    print("✅ Тест 3 пройден: Все этапы работают с общим store")
    print(f"   Store после всех этапов: {result['store']}")


@pytest.mark.asyncio
async def test_04_store_persists_across_agent_chain(migrated_db,  agent_factory, test_helpers, unique_id, agent_repo, mock_llm):
    """
    ...
    agent_a = AgentConfig(
        ...
        name="Agent A",
        description="Первый агент в цепочке",
...
@tool
async def mark_visited_a(agent_name: str) -> str:
...
""",
    ...
    agent_b = AgentConfig(
        ...
        name="Agent B",
        description="Второй агент в цепочке",
        type=AgentType.REACT,
        prompt="""
...
@tool
async def mark_visited_b(agent_name: str) -> str:
    \"\"\"Отмечает что агент был вызван\"\"\"
    state = get_state()
    state["store"][f"{agent_name}_visited"] = True
...
""",
    ...
    agent_c = AgentConfig(
        ...
        name="Agent C",
        description="Третий агент в цепочке",
        type=AgentType.REACT,
        prompt="""
...
""",
    ...
    coordinator_config = AgentConfig(
        ...
        description="Координатор цепочки агентов",
        type=AgentType.REACT,
        prompt="""
...
""",
    ...
    if global_mock:
        ...
        global_mock.configure(
            response_queue=[
                # Координатор: вызов agent_a
                {"type": "tool_call", "tool": "agent_a", "args": {"request": "Выполни свою задачу"}},
                # agent_a: вызов mark_visited_a
                {"type": "tool_call", "tool": "mark_visited_a", "args": {"agent_name": "agent_a"}},
                # agent_a: завершение
                {"type": "text", "content": "Agent A отметился. Store обновлен."},
                # Координатор: вызов agent_b
                {"type": "tool_call", "tool": "agent_b", "args": {"request": "Выполни свою задачу"}},
                # agent_b: вызов mark_visited_b
                {"type": "tool_call", "tool": "mark_visited_b", "args": {"agent_name": "agent_b"}},
                # agent_b: завершение
                {"type": "text", "content": "Agent B отметился. Store обновлен."},
                # Координатор: вызов agent_c
                {"type": "tool_call", "tool": "agent_c", "args": {"request": "Выполни свою задачу"}},
                # agent_c: завершение (не вызывает инструментов)
                {"type": "text", "content": "Agent C видит: A=True, B=True"},
                # Координатор: завершение
                {"type": "text", "content": "Цепочка выполнена"}
            ]
        )
    
    # Теперь создаем координатора (он будет использовать уже настроенный мок)
    coordinator = await agent_factory.get_agent("test_chain_coordinator")
    
    session_id = unique_id("thread")
    config = {"configurable": {"session_id": session_id}}
    
    session_id_for_test = unique_id("chain_test")
    ...
    # Проверяем что store накопил данные от ВСЕХ агентов в точности
    store = result["store"]
    
    # Agent A должен был сохранить свои данные
    assert "agent_a_visited" in store, "Agent A должен был сохранить agent_a_visited"
    assert store["agent_a_visited"] is True, "agent_a_visited должен быть True"
    assert "agent_a_timestamp" in store, "Agent A должен был сохранить agent_a_timestamp"
    assert store["agent_a_timestamp"] == "2025-10-13 15:30:00", f"agent_a_timestamp должен быть '2025-10-13 15:30:00', получен: {store.get('agent_a_timestamp')}"
    
    # Agent B должен был сохранить свои данные
    assert "agent_b_visited" in store, "Agent B должен был сохранить agent_b_visited"
    assert store["agent_b_visited"] is True, "agent_b_visited должен быть True"
    assert "agent_b_timestamp" in store, "Agent B должен был сохранить agent_b_timestamp"
    assert store["agent_b_timestamp"] == "2025-10-13 15:31:00", f"agent_b_timestamp должен быть '2025-10-13 15:31:00', получен: {store.get('agent_b_timestamp')}"
    
    # Проверяем что в store нет лишних данных
    expected_keys = {"agent_a_visited", "agent_a_timestamp", "agent_b_visited", "agent_b_timestamp"}
    actual_keys = set(store.keys())
    unexpected_keys = actual_keys - expected_keys
    assert not unexpected_keys, f"В store найдены неожиданные ключи: {unexpected_keys}"
    
    print(f"✅ Тест 4 пройден: Store персистится через всю цепочку. Store: {store}")


@pytest.mark.asyncio
async def test_05_initial_store_from_flow_config(migrated_db,  agent_factory, test_helpers, unique_id, agent_repo, flow_repo, mock_llm):
    """
...
""",
    ...
    # Теперь создаем агента (он будет использовать уже настроенный мок)
    ...
    print("✅ Тест 5 пройден: Начальные значения store из FlowConfig работают")
    print(f"   Initial store: {result_1['store']}")


@pytest.mark.asyncio
async def test_06_store_merge_not_overwrite(migrated_db,  agent_factory, test_helpers, unique_id, agent_repo, flow_repo, mock_llm):
    """
...
@tool
async def update_settings(timeout: int, theme: str) -> str:
...
""",
    ...
    # Настраиваем mock_llm ПЕРЕД созданием агента
    from core.clients.llm import get_llm, get_global_mock_llm
    
    # Создаем мок если его еще нет (вызов get_llm создаст его)
    _ = get_llm("mock-gpt-4")
    ...
    print("✅ Тест 6 пройден: Store из FlowConfig мержится корректно")


@pytest.mark.asyncio
async def test_07_react_agent_session_set_then_prompt_substitution(migrated_db,  agent_factory, unique_id, agent_repo, flow_repo, mock_llm):
    """
...
""",
        tools=[
            ToolReference(tool_id="apps.agents.tools.session.session_tools.session_set")
        ],
        llm_config=LLMConfig(model="mock-gpt-4", temperature=0.1, context_window=10000),
    )
...
""",
    ...
    if "warehouse_id" not in result_a.get("store", {}):
        ...
        from apps.agents.tools.session.session_tools import session_set as session_set_tool
    ...
    print("✅ Тест 7 пройден: ReAct Agent B получил переменные из store в промпте!")
