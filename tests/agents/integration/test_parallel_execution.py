...
class TestParallelNodeExecution:
    ...
    async def setup_parallel_flow(self, app, container, unique_id):
        ...
        Каждая нода делает sleep 0.5 сек и записывает timestamp.
        Если выполнение параллельное - общее время ~0.5 сек.
        Если последовательное - общее время ~1.5 сек.
        """
        agent_id = f"parallel_test_{unique_id}"
...
""",
    ...
    async def test_fanout_nodes_execute_in_parallel(
        ...
        """
        Тест: 3 ноды с sleep 0.5 сек выполняются параллельно.
        
        Ожидаемое поведение:
        - Общее время ~0.5-1.0 сек (параллельно)
        - НЕ ~1.5+ сек (последовательно)
        """
        agent_id = setup_parallel_flow
        session_id = f"{agent_id}:parallel-{unique_id}-{uuid.uuid4().hex[:8]}"
    ...
    async def test_parallel_nodes_timestamps_overlap(
        ...
        """
        agent_id = setup_parallel_flow
        session_id = f"{agent_id}:timestamps-{unique_id}-{uuid.uuid4().hex[:8]}"
...
class TestParallelNodesMerge:
    ...
    async def setup_merge_flow(self, app, container, unique_id):
        ...
        """
        agent_id = f"merge_test_{unique_id}"
...
""",
    ...
    async def test_different_fields_preserved_after_merge(
        ...
        """
        agent_id = setup_merge_flow
        session_id = f"{agent_id}:merge-{unique_id}-{uuid.uuid4().hex[:8]}"
