...
from tests.agents.fixtures.external_agent.main import external_agent_app
...
class TestAgentDiscoveryService:
    ...
    async def test_register_and_list_agents(self, app):
        ...
        all_agents = await container.agent_repository.list_all()
        assert len(all_agents) >= 1

        found = await container.agent_repository.get("test_agent_1")
    ...
    async def test_list_active_agents(self, app):
        ...
        await container.agent_repository.set(agent_active)
        await container.agent_repository.set(agent_inactive)

        # Фильтруем external агентов по статусу
        all_agents = await container.agent_repository.list_all()
        active_agents = [a for a in all_agents if a.type == AgentType.EXTERNAL and a.status == ExternalAgentStatus.ACTIVE]
    ...
    async def test_get_agent_by_url(self, app, unique_id):
        ...
        agent_id = f"url_test_agent_{unique_id}"
        ...
        found = None
        for a in all_agents:
            if a.type == AgentType.EXTERNAL and a.url == url:
                found = a
                break
        assert found is not None
    ...
    async def test_update_health_check(self, app):
        ...
        agent = await container.agent_repository.get("health_test_agent")
        ...
        updated = await container.agent_repository.get("health_test_agent")
...
class TestExternalAgentsAPI:
    ...
    async def test_list_external_agents_empty(self, client):
        ...
        from apps.agents.src.models import AgentType
        response = await client.get("/agents/api/v1/agents/", params={"type": AgentType.EXTERNAL.value})
        assert response.status_code == 200
    ...
    async def test_register_and_get_external_agent(self, app, client, asgi_external_agent):
        ...
        response = await client.get("/agents/api/v1/agents/api_test_agent")
        assert response.status_code == 200
    ...
    async def test_delete_external_agent(self, app, client):
        ...
        # API для external agents теперь объединен с основным agents API
        response = await client.delete("/agents/api/v1/agents/delete_test_agent")
        assert response.status_code == 200

        deleted = await container.agent_repository.get("delete_test_agent")
        assert deleted is None

    @pytest.mark.asyncio
    async def test_get_nonexistent_agent(self, client):
        """404 для несуществующего агента."""
        response = await client.get("/agents/api/v1/agents/external/nonexistent")
