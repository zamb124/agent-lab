...
from apps.agents.src.agent.nodes import ReactNode, CodeNode
from apps.agents.src.agent.agent import Agent
from apps.agents.src.models import ResourceType, ResourceReference
...
class TestCodeResourceWithReactNode:
    """
    CODE resource с ReactNode: функции доступны в tools.
    """

    @pytest.mark.asyncio
    async def test_code_resource_in_react_tool(self, mock_llm_with_queue):
        """
        Code resource доступен в inline tool ReactNode.
        """
        code_resource = {
            "type": "code",
            "config": {
...
class TestHTTPResourceWithReactNode:
    """
    HTTP resource с ReactNode: HTTP запросы в tools.
    """

    @pytest.mark.asyncio
    async def test_http_resource_in_react_tool(self, mock_llm_with_queue):
        """
        HTTP resource доступен в inline tool ReactNode.
        """
        http_resource = {
            "type": "http",
            "config": {
...
class TestResourceInheritance:
    """
    Наследование ресурсов: agent_resources -> node_resources.
    """

    @pytest.mark.asyncio
    async def test_agent_resources_available_in_code_node(self):
        """
        Ресурсы из agent_resources доступны в CodeNode через state.
        """
        code_resource = {
            "type": "code",
            "config": {
...
# =============================================================================
# PROMPT Resource с ReactNode
# =============================================================================

class TestPromptResourceWithReactNode:
    """
    PROMPT resource с ReactNode: шаблоны в tools.
    """

    @pytest.mark.asyncio
    async def test_prompt_resource_in_react_tool(self, mock_llm_with_queue):
        """
        Prompt resource доступен в inline tool ReactNode.
        """
        prompt_resource = {
            "type": "prompt",
            "config": {
...
""",
    ...
    async def test_files_resource_in_react_tool(self, mock_llm_with_queue, unique_prefix):
        """
        FILES resource доступен в inline tool ReactNode.
        """
        files_resource = {
            "type": "files",
            "config": {
...
""",
    ...
    async def test_cache_resource_in_react_tool(self, mock_llm_with_queue, unique_namespace):
        """
        CACHE resource доступен в inline tool ReactNode.
        """
        cache_resource = {
            "type": "cache",
            "config": {
...
""",
    ...
    async def test_llm_resource_in_react_tool(self, mock_llm_with_queue):
        """
        LLM resource доступен в inline tool ReactNode.
        """
        llm_resource = {
            "type": "llm",
            "config": {
...
class TestResourceHierarchy:
    """
    Иерархия ресурсов: agent_config.resources -> skill.resources -> node.resources
    """

    @pytest.mark.asyncio
    async def test_agent_level_resources_in_react_tool(self, mock_llm_with_queue):
        """
        Resources на уровне агента доступны в tools ReactNode.
        """
        code_resource = {
            "type": "code",
            "config": {
...
"""
        ...
        # Создаём agent_config с resources
        agent_config = {
            "resources": {
        ...
        state = make_state(content="Double 21", agent_config=agent_config)
        result = await react_node.run(state)
...
"""
        ...
        # agent_config с skill resources
        agent_config = {
            "skills": {
                "math_skill": {
                    "resources": {
...
""",
        ...
        state = make_state(input=10, skill_id="math_skill", agent_config=agent_config)
        result = await node.run(state)
...
""",
        ...
        state = make_state(input=5, skill_id="premium", agent_config=agent_config)
        result = await node.run(state)
...
"""
        ...
        # В реальном сценарии agent_config передаётся в state
...
class TestSharedResources:
    ...
    async def test_shared_resource_from_db(self, container):
        ...
        """
        from apps.agents.src.models import ResourceDefinition
