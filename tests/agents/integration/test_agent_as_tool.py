...
Сценарий:
1. main_flow с ReactNode, tool: support_agent
...
from apps.agents.src.models import NodeConfig, LLMConfig, ToolReference
from apps.agents.src.tools import BaseTool, InlineTool
...
class TestFlowWithSubagentInterrupt:
    ...
    main_flow (ReactNode)
        ↓ вызывает tool
    support_agent (inline react_node)
        ↓ вызывает tool
    ask_user (inline tool с AgentInterrupt)
    """
...
'''
    ...
    async def test_flow_with_subagent_interrupt(
        ...
        flow = await Agent.from_config(main_agent_config)
...
'''
    ...
    async def test_tool_result_has_correct_role(self):
        ...
        """
        from apps.agents.src.agent.runners.react_runner import new_tool_result_message
    ...
    async def test_full_resume_cycle(
        ...
        flow = await Agent.from_config(main_flow_with_subagent)
...
""",
    ...
    async def test_tool_registry_creates_node_as_tool(self, app):
        """ToolRegistry создаёт NodeAsToolWrapper из inline react_node конфига."""
        container = get_container()
...
'''
    ...
    async def test_files_available_in_subagent(
        ...
        flow = await Agent.from_config(agent_with_file_tool)
        state = ExecutionState(
            task_id="test-task",
            context_id="test-context",
            user_id="test-user",
