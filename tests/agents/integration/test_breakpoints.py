...
class TestBreakpointsReactAgent:
    ...
    @pytest_asyncio.fixture
    async def flow(self, app) -> Agent:
        """Создаёт Agent из example_react."""
        container = get_container()
        return await container.agent_factory.get_flow("example_react")
...
class TestBreakpointsAPIIntegration:
    ...
    async def setup_breakpoint_flow(self, app, container, unique_id, mock_context):
        ...
        agent_id = f"bp_api_test_{unique_id}"
    ...
    async def test_breakpoint_via_taskiq_stops_execution(
        ...
        API -> process_agent_task -> process_task -> ExecutionState -> Agent
        """
        from apps.agents.src.tasks.agent_tasks import process_agent_task

        agent_id = setup_breakpoint_flow
        session_id = f"{agent_id}:bp-taskiq-{unique_id}"
    ...
    async def test_continue_after_breakpoint_via_taskiq(
        ...
        agent_id = setup_breakpoint_flow
        session_id = f"{agent_id}:bp-continue-{unique_id}"
    ...
    async def test_no_breakpoint_completes_normally(
        ...
        agent_id = setup_breakpoint_flow
        session_id = f"{agent_id}:bp-none-{unique_id}"
