...
async def check_node(state):
    ...
    if "user_name" not in state["store"]:
        # Проверяем есть ли interrupt_context
        if state.get("interrupt_context"):
            # Пользователь уже ответил
            messages = state.get("messages", [])
            for msg in reversed(messages):
                if isinstance(msg, HumanMessage):
                    state["store"]["user_name"] = msg.content.strip()
                    state.pop("interrupt_context", None)
                    break
        else:
            # Спрашиваем у пользователя через interrupt
            raise AgentInterrupt("Как вас зовут?")
...
""",
    ...
    try:
        ...
        # Сохраняем состояние с ответом пользователя в БД (interrupt_context остается!)
        await state_manager.save_session(saved_state)
        ...
        result = await agent.ainvoke(
            {"messages": [HumanMessage(content="Иван")], "session_id": session_id},
