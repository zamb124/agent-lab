...
def _check_tavily_api_key():
    """Проверяет наличие Tavily API ключа в конфиге или env"""
    ...
    try:
        ...
        if hasattr(settings, 'sgr') and settings.sgr and settings.sgr.tavily_api_key:
            return True
    except Exception:
        pass
    return False
...
requires_real_api = pytest.mark.skipif(
    not _check_tavily_api_key(),
    reason="Требуется TAVILY_API_KEY для реальных API тестов (в env или conf.json)"
)
...
async def test_research_flow_migration(migrated_db,  flow_repo, agent_repo, migrator, test_company):
    ...
    assert research_flow.store["max_iterations"] == 1
    ...
    assert len(graph_def.nodes) == 7, f"Ожидали 7 нод, получили {len(graph_def.nodes)}"
    
    node_ids = [node.id for node in graph_def.nodes]
    expected_nodes = ["analyze", "search", "process", "extract", "synthesize", "check", "return_report"]
