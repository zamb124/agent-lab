...
@tool
async def stategraph_test_tool(message: str) -> str:
    """Тестовая функция, которая изменяет state"""
    logger.info(f"stategraph_test_tool вызвана с: {message}")
...
async def test_stategraph_tool(test_user, test_company):
    ...
    result = await stategraph_test_tool.ainvoke({"message": "Привет StateGraph!", "state": mock_state})
