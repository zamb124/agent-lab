...
async def _open_settings_agent_tab(page: Page, *, origin: str) -> None:
...
async def test_f1_settings_agent_tab_visible(
    ...
    await scenario.step("Вкладка HumanitecAgent видна на странице настроек", ui_page_system)
...
async def test_f2_settings_agent_connect_section(
    ...
    _ = frontend_ui
    await _open_settings_agent_tab(ui_page_system, origin=frontend_ui.origin)
    await expect(
    ...
    await scenario.step("Секция подключения HumanitecAgent отображается", ui_page_system)
...
async def test_f3_settings_agent_devices_empty(
    ...
    await scenario.step("Пустой список устройств отображается", ui_page_system)
...
async def test_f4_settings_agent_audit_empty(
    ...
    await scenario.step("Пустой audit log отображается", ui_page_system)
...
async def test_f5_settings_agent_pairing_code(
    ...
    await expect(
        ui_page_system.get_by_text(re.compile(r"Код pairing|Pairing code"))
    ).to_be_visible(timeout=30_000)
    ...
    await scenario.step("Pairing code создан и отображается", ui_page_system)
...
async def test_f6_settings_agent_download_link(
    ...
    await scenario.step("Ссылка на download page видна", ui_page_system)
...
async def test_f7_settings_agent_release_banner(
    ...
    await scenario.step("Блок статуса релиза отображается", ui_page_system)
...
async def test_f8_settings_agent_help_text(
    ...
    await scenario.step("Help-текст про pairing отображается", ui_page_system)
