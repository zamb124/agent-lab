/**
 * Построчный fallback Markdown для flows-chat (без globalThis.marked).
 */
...
import {
    ...
    flowsChatMarkdownToHtml,
} from '@platform/lib/flows-chat/markdown.js';

function fallbackFromRaw(md) {
    return fallbackMarkdownToHtml(escapeHtmlAngleAndAmp(md));
}

describe('flows-chat markdown fallback', () => {
    ...
    it('flowsChatMarkdownToHtml в режиме streaming не вызывает marked.parse', () => {
        ...
        try {
            const html = flowsChatMarkdownToHtml('## Часть\n\ntекст', { streaming: true });
