...
class TestScoreCalculationFormula:
    ...
    @pytest.mark.asyncio
    async def test_formula_zero_degree_coef_1(self, graph_service):
        """relevance=0.5, degree=0, coef=1.0 -> score=0.5"""
        score, _ = graph_service._calculate_entity_score(0.5, 0, 1.0)
        assert score == 0.5
    
    @pytest.mark.asyncio
    async def test_formula_zero_degree_coef_12(self, graph_service):
        """relevance=0.5, degree=0, coef=1.2 -> score=0.6"""
        score, _ = graph_service._calculate_entity_score(0.5, 0, 1.2)
        assert score == 0.6
    
    @pytest.mark.asyncio
    async def test_formula_degree_1(self, graph_service):
        """relevance=0.5, degree=1, coef=1.0 -> score=1.0"""
        score, _ = graph_service._calculate_entity_score(0.5, 1, 1.0)
        assert score == 1.0
    
    @pytest.mark.asyncio
    async def test_formula_degree_2(self, graph_service):
        """relevance=0.5, degree=2, coef=1.0 -> score=1.5"""
        score, _ = graph_service._calculate_entity_score(0.5, 2, 1.0)
        assert score == 1.5
    
    @pytest.mark.asyncio
    async def test_formula_degree_4_coef_11(self, graph_service):
        """relevance=0.6, degree=4, coef=1.1 -> score=3.3"""
        score, _ = graph_service._calculate_entity_score(0.6, 4, 1.1)
        # 0.6 * (1+4) * 1.1 = 0.6 * 5 * 1.1 = 3.3
        assert score == 3.3
    
    @pytest.mark.asyncio
    async def test_formula_high_relevance(self, graph_service):
        """relevance=1.0, degree=3, coef=1.2 -> score=4.8"""
        score, _ = graph_service._calculate_entity_score(1.0, 3, 1.2)
        # 1.0 * (1+3) * 1.2 = 1.0 * 4 * 1.2 = 4.8
        assert score == 4.8
    
    @pytest.mark.asyncio
    async def test_formula_low_relevance(self, graph_service):
        """relevance=0.2, degree=2, coef=0.8 -> score=0.48"""
        score, _ = graph_service._calculate_entity_score(0.2, 2, 0.8)
        # 0.2 * (1+2) * 0.8 = 0.2 * 3 * 0.8 = 0.48
        assert score == 0.48
    
    @pytest.mark.asyncio
    async def test_formula_fractional_coef(self, graph_service):
        """relevance=0.8, degree=1, coef=0.9 -> score=1.44"""
        score, _ = graph_service._calculate_entity_score(0.8, 1, 0.9)
...
class TestSizeCalculation:
    ...
    @pytest.mark.asyncio
    async def test_size_minimum_bound(self, graph_service):
        """При очень низком score size >= 15"""
        _, size = graph_service._calculate_entity_score(0.01, 0, 0.5)
        assert size >= 15.0
        assert size <= 16.0  # Близко к минимуму
    
    @pytest.mark.asyncio
    async def test_size_maximum_bound(self, graph_service):
        """При очень высоком score size = 50"""
        _, size = graph_service._calculate_entity_score(1.0, 100, 2.0)
        assert size == 50.0
    
    @pytest.mark.asyncio
    async def test_size_score_05(self, graph_service):
        """score=0.5 -> size=15+5=20"""
        score, size = graph_service._calculate_entity_score(0.5, 0, 1.0)
        assert score == 0.5
        assert size == 20.0
    
    @pytest.mark.asyncio
    async def test_size_score_1(self, graph_service):
        """score=1.0 -> size=15+10=25"""
        score, size = graph_service._calculate_entity_score(0.5, 1, 1.0)
        assert score == 1.0
        assert size == 25.0
    
    @pytest.mark.asyncio
    async def test_size_score_2(self, graph_service):
        """score=2.0 -> size=15+20=35"""
        score, size = graph_service._calculate_entity_score(1.0, 1, 1.0)
        assert score == 2.0
        assert size == 35.0
    
    @pytest.mark.asyncio
    async def test_size_score_35(self, graph_service):
        """score=3.5 -> size=15+35=50 (capped)"""
        score, size = graph_service._calculate_entity_score(1.0, 2.5, 1.0)
        # 1.0 * (1+2.5) * 1.0 = 3.5
        assert score == 3.5
...
class TestCountDegrees:
    ...
    @pytest.mark.asyncio
    async def test_empty_list(self, graph_service):
        """Пустой список -> пустой dict"""
        result = graph_service._count_degrees([])
        assert result == {}
    
    @pytest.mark.asyncio
    async def test_single_relationship(self, crm_container, test_context, unique_id):
        ...
        e1 = await entity_service.create_entity(EntityCreate(
            type="person",
            name=f"Degree Test 1 {unique_id('deg')}",
            attributes={},
        ))
        e2 = await entity_service.create_entity(EntityCreate(
            type="person",
            name=f"Degree Test 2 {unique_id('deg')}",
            attributes={},
        ))
    ...
    @pytest.mark.asyncio
    async def test_hub_entity_multiple_connections(self, crm_container, test_context, unique_id):
        ...
        for i in range(3):
            sat = await entity_service.create_entity(EntityCreate(
                type="person",
                name=f"Satellite {i} {unique_id('sat')}",
                attributes={},
            ))
...
class TestEntityRelevance:
    ...
    @pytest.mark.asyncio
    async def test_default_relevance_05(self, crm_container, test_context, unique_id):
        """Без указания relevance -> 0.5"""
        entity_service = crm_container.entity_service
        
        entity = await entity_service.create_entity(EntityCreate(
            type="person",
            name=f"Default Rel {unique_id('rel')}",
            attributes={},
        ))
    ...
    @pytest.mark.asyncio
    async def test_custom_relevance_095(self, crm_container, test_context, unique_id):
        """relevance=0.95 сохраняется"""
        entity_service = crm_container.entity_service
        
        entity = await entity_service.create_entity(EntityCreate(
            type="person",
            name=f"High Rel {unique_id('rel')}",
            attributes={},
            relevance=0.95,
        ))
    ...
    @pytest.mark.asyncio
    async def test_custom_relevance_02(self, crm_container, test_context, unique_id):
        """relevance=0.2 сохраняется"""
        entity_service = crm_container.entity_service
        
        entity = await entity_service.create_entity(EntityCreate(
            type="person",
            name=f"Low Rel {unique_id('rel')}",
            attributes={},
            relevance=0.2,
        ))
...
class TestGraphIntegration:
    ...
    async def score_test_data(self, crm_container, test_context, unique_id):
        """
        ...
        secondary = await entity_service.create_entity(EntityCreate(
            type="person", 
            name=f"Secondary {unique_id('score')}",
            attributes={},
            relevance=0.4,
        ))
    ...
    @pytest.mark.asyncio
    async def test_graph_nodes_contain_score_fields(self, graph_service, score_test_data, test_context):
    ...
    @pytest.mark.asyncio
    async def test_main_has_degree_2(self, graph_service, score_test_data, test_context):
    ...
    @pytest.mark.asyncio
    async def test_secondary_has_degree_1(self, graph_service, score_test_data, test_context):
    ...
    @pytest.mark.asyncio
    async def test_relevance_preserved_in_graph(self, graph_service, score_test_data, test_context):
    ...
    @pytest.mark.asyncio
    async def test_main_has_higher_score_than_secondary(self, graph_service, score_test_data, test_context):
    ...
    @pytest.mark.asyncio
    async def test_main_has_larger_size_than_secondary(self, graph_service, score_test_data, test_context):
