...
class TestCRMStaticCSS:
    ...
    @pytest.mark.asyncio
    async def test_crm_css_available(self, frontend_client):
        """crm.css доступен"""
        response = await frontend_client.get("/static/crm/css/crm.css")
        
        assert response.status_code == 200
        assert "text/css" in response.headers.get("content-type", "")

    @pytest.mark.asyncio
    async def test_crm_css_contains_variables(self, frontend_client):
    ...
    @pytest.mark.asyncio
    async def test_crm_css_has_dark_theme(self, frontend_client):
...
class TestCRMStaticJS:
    ...
    @pytest.mark.asyncio
    async def test_crm_module_js_available(self, frontend_client):
        """crm.module.js доступен"""
        response = await frontend_client.get("/static/crm/js/crm.module.js")
        
        assert response.status_code == 200
        assert "javascript" in response.headers.get("content-type", "")

    @pytest.mark.asyncio
    async def test_crm_module_js_contains_api_client(self, frontend_client):
    ...
    @pytest.mark.asyncio
    async def test_crm_module_js_contains_module_class(self, frontend_client):
    ...
    @pytest.mark.asyncio
    async def test_crm_module_js_exposes_global(self, frontend_client):
    ...
    @pytest.mark.asyncio
    async def test_vis_network_js_available(self, frontend_client):
        """vis-network.min.js доступен (локальная версия)"""
        response = await frontend_client.get("/static/crm/js/vis-network.min.js")
        
        assert response.status_code == 200
        assert "javascript" in response.headers.get("content-type", "")

    @pytest.mark.asyncio
    async def test_vis_network_js_is_valid(self, frontend_client):
...
class TestCRMStaticImages:
    ...
    @pytest.mark.asyncio
    async def test_logo_svg_available(self, frontend_client):
        """Логотип Networkle доступен"""
        response = await frontend_client.get("/static/crm/img/logo_networkle.svg")
...
class TestCRMStaticNotFound:
    ...
    @pytest.mark.asyncio
    async def test_nonexistent_css_returns_404(self, frontend_client):
        """Несуществующий CSS возвращает 404"""
        response = await frontend_client.get("/static/crm/css/nonexistent.css")
        
        assert response.status_code == 404

    @pytest.mark.asyncio
    async def test_nonexistent_js_returns_404(self, frontend_client):
        """Несуществующий JS возвращает 404"""
        response = await frontend_client.get("/static/crm/js/nonexistent.js")
