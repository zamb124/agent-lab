...
class TestErrorPages:
    ...
    async def test_nonexistent_page_handled(self, page: Page, server_url: str):
        """Несуществующая страница обрабатывается (редирект на auth или 404)"""
        response = await page.goto(f"{server_url}/frontend/nonexistent-page-12345")
        
        assert response.status in [200, 302, 404], f"Страница должна быть обработана, получен {response.status}"
