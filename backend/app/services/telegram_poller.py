...
class TelegramPoller:
    ...
    async def _discover_bots(self):
        ...
        # Получаем все flows
        flows_to_check = ["test_flow", "smart_flow", "weather_flow"]  # TODO: get_all_flows()
        ...
        for flow_id in flows_to_check:
            try:
                ...
                token = await TelegramInterface.get_bot_token_for_flow(flow_id, telegram_config)
                if not token:
                    continue
                ...
                try:
                    ...
                    logger.warning(f"⚠️ Не удалось установить команды для @{username}: {cmd_error}")
            ...
            try:
                ...
                async with httpx.AsyncClient(timeout=35.0) as client:
                    ...
                    if response.status_code != 200:
                        logger.error(f"❌ Ошибка Telegram API для @{username}: {response.status_code}")
                    ...
                    for update in updates:
                        try:
                            await self._process_update(update, flow_id, platform_config, token)
    ...
    async def _process_update(self, update: Dict, flow_id: str, platform_config: Dict, token: str):
        ...
        try:
            ...
            webhook_url = f"http://localhost:{settings.port}/api/v1/webhook/telegram/{flow_id}"
            
            async with httpx.AsyncClient() as client:
                response = await client.post(webhook_url, json=update)
                
                if response.status_code == 200:
                    logger.info(f"📋 Long polling: update {update.get('update_id')} отправлен в webhook")
                else:
                    logger.error(f"❌ Ошибка отправки в webhook: {response.status_code}")
