...
from app.api.v1 import webhooks, admin, telegram, tokens, auth, flows, fashn
...
logging.getLogger("app.agents.base").setLevel(logging.WARNING)  # Убираем избыточные логи загрузки tools
logging.getLogger("app.workers.task_processor").setLevel(logging.INFO)  # Основные логи воркера
logging.getLogger("app.tools.standard").setLevel(logging.WARNING)  # Убираем технические логи ask_user
logging.getLogger("app.core.llm_factory").setLevel(logging.WARNING)  # Отключаем wrapper логи
...
async def _periodic_cleanup():
    ...
    while True:
        try:
            ...
            if deleted_count > 0:
                logger.info(f"🧹 Периодическая очистка: удалено {deleted_count} истекших записей")
...
@app.get("/")
async def root():
    """Корневой эндпоинт - редирект на фронтенд"""
