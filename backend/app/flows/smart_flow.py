...
def router_function(state: RouterState) -> RouterState:
    ...
    # Выбираем агента
    if any(kw in user_input.lower() for kw in ["посчитай", "сколько", "+", "-", "*", "/"]):
...
async def explainer_node(state: RouterState) -> RouterState:
    ...
    agent_type = "калькулятор" if state["selected_agent"] == "calculator" else "погодный"
    agent_result = state["messages"][-1].content if state["messages"] else "нет результата"
