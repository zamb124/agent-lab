...
async def virtual_try_on(
    ...
    """
    try:
        logger.info(f"Начинаем виртуальную примерку: модель={model_image_file_id}, продукт={product_image_file_id}")
        ...
        if model_bytes is None:
            return f"❌ Не удалось скачать файл модели из S3"
            
        product_bytes = await s3_client.download_bytes(product_record.s3_key)
        if product_bytes is None:
            return f"❌ Не удалось скачать файл продукта из S3"
        ...
        if variations > 0:
            ...
            model_url = await fashn_client._upload_image(model_bytes, "model_for_variations.png")
        ...
        logger.info(f"Виртуальная примерка завершена, создано {len(all_results)} результатов")
