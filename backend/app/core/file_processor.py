...
class FileProcessor:
    ...
    async def process_file_from_url(
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                raise ValueError(f"Не удалось скачать файл по URL: {response.status_code}")
    ...
    async def get_file_record(self, file_id: str, provider: Optional[str] = None) -> Optional[FileRecord]:
        """
    ...
    def format_file_message(self, file_record: FileRecord) -> str:
        ...
        size_str = f"{size_mb:.2f} MB" if size_mb >= 1 else f"{file_record.file_size} байт"
