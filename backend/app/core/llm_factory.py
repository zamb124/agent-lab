...
from app.core.config import settings, LLMProviderConfig
...
def get_llm(provider: Optional[str] = None, model: Optional[str] = None) -> BaseLLM:
    ...
    try:
        if provider_name == "openai":
            llm = _create_openai_llm(provider_config, final_model)
...
def _create_openai_llm(provider_config: LLMProviderConfig, model: str) -> BaseLLM:
    """Создает OpenAI LLM"""
    try:
        kwargs = {
            "model": model,
        ...
        raise ImportError("Для использования OpenAI установите: pip install langchain-openai")
...
def _create_anthropic_llm(provider_config: LLMProviderConfig, model: str) -> BaseLLM:
    ...
    try:
        ...
        raise ImportError("Для использования Anthropic установите: pip install langchain-anthropic")
...
def _create_ollama_llm(provider_config: LLMProviderConfig, model: str) -> BaseLLM:
    ...
    try:
        ...
        model_config = provider_config.models.get(model, {})
        if "max_tokens" in model_config:
            kwargs["num_predict"] = model_config["max_tokens"]  # Ollama использует num_predict
        
        return Ollama(**kwargs)
        
    except ImportError:
        raise ImportError("Для использования Ollama установите: pip install langchain-community")
...
def _create_mock_llm(provider_config: LLMProviderConfig, model: str) -> BaseLLM:
    ...
    class MockLLM(BaseChatModel):
        ...
        def __init__(self, model, temperature, **kwargs):
            ...
            object.__setattr__(self, 'default_response', "Я обработаю ваш запрос используя доступные инструменты.")
