...
from app.core.models import GraphDefinition, NodeType, LLMConfig, ToolReference, ConditionType, CodeMode
...
class GraphBuilder:
    ...
    async def build_from_definition(
        ...
        """
        logger.info(f"Строим граф с {len(graph_def.nodes)} нодами и {len(graph_def.edges)} ребрами")
        ...
        for source, mapping in conditional_groups.items():
            ...
            if (source_node and 
                hasattr(source_node, 'code_mode') and 
                source_node.code_mode == CodeMode.INLINE_CODE and 
                source_node.inline_code):
                ...
                try:
                    ...
                    for cond_name in condition_names:
                        if cond_name in local_namespace:
                            ...
                            logger.info(f"✅ INLINE функция условия найдена: {cond_name}")
                            break
                            
                except Exception as e:
                    logger.error(f"❌ Ошибка извлечения функции условия из inline кода: {e}")
            
            # Если не нашли в inline коде, ищем по пути
            if not condition_func:
                for edge in graph_def.edges:
                    if edge.source == source and edge.condition_type == ConditionType.ROUTER and edge.condition:
                        try:
                            ...
                            logger.warning(f"Не удалось импортировать функцию условия {edge.condition}: {e}")
    ...
    async def _create_agent_node(self, node, llm_config: Optional[LLMConfig] = None):
        ...
        if not agent_id:
            raise ValueError(f"Нода агента {node.id} должна содержать agent_id или function_class")
    ...
    async def _create_tool_node(self, node):
        ...
        tool_ref = ToolReference(tool_id=tool_id, params=node.params.get("tool_params", {}))
        ...
        async def tool_node(state: GraphState) -> GraphState:
            ...
            try:
                ...
                # Вызываем инструмент
                result = await tool.ainvoke(input_data) if hasattr(tool, 'ainvoke') else tool.invoke(input_data)
    ...
    async def _create_function_node(self, node):
        ...
        if hasattr(node, 'code_mode') and node.code_mode == CodeMode.INLINE_CODE:
            ...
            if not function_path:
                raise ValueError(f"Нода функции {node.id} должна содержать function или function_path")
        ...
        try:
            ...
            for func_name in possible_names:
                if func_name in local_namespace:
                    ...
                    logger.info(f"✅ INLINE функция найдена: {func_name} для ноды {node.id}")
                    return func
            
            # Если не нашли по именам, берем первую функцию
            for name, obj in local_namespace.items():
                if callable(obj) and not name.startswith('_'):
                    logger.info(f"✅ INLINE функция найдена (первая): {name} для ноды {node.id}")
                    return obj
