...
def load_json_config(config_path: Union[str, Path]) -> Dict[str, Any]:
    ...
    if not config_path.exists():
        logger.info(f"Файл конфигурации {config_path} не найден, используем значения по умолчанию")
        return {}
...
def get_config_paths() -> list[Path]:
    ...
    project_root = backend_dir.parent  # agent-lab/
    
    config_paths = [
        # Базовая конфигурация в backend/
        backend_dir / "conf.json",
...
def get_env_or_config(env_key: str, config_key: str, config: Dict[str, Any], default: Any = None) -> Any:
    """
...
def resolve_config_value(field_name: str, field_type: type, json_config: Dict[str, Any], config_path: str) -> Any:
    """
