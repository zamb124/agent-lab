...
class S3Client:
    ...
    def __init__(
        ...
        self._session = None
        self._client = None
        self._storage = Storage() if track_files else None
    ...
    async def list_objects(
        ...
        try:
            ...
            logger.info(f"✅ Получен список объектов S3: {bucket} (найдено {len(objects)})")
            return objects
    ...
    async def generate_presigned_url(
        ...
        try:
            ...
            logger.info(f"✅ Создан presigned URL для {bucket}/{key} (срок: {expiration}s)")
            return url
            ...
            logger.error(f"❌ Неожиданная ошибка при создании presigned URL для {key}: {e}")
            return None
    ...
    async def copy_object(
        ...
        try:
            ...
            logger.info(f"✅ Объект скопирован в S3: {source_bucket}/{source_key} -> {dest_bucket}/{dest_key}")
            return True
...
async def get_default_s3_client() -> Optional[S3Client]:
    ...
    if _default_s3_client is None:
        ...
        if hasattr(settings, 's3') and settings.s3.enabled and settings.s3.default_bucket:
            _default_s3_client = S3ClientFactory.create_client_for_bucket(settings.s3.default_bucket)
            logger.info(f"✅ Инициализирован дефолтный S3 клиент для бакета: {settings.s3.default_bucket}")
        else:
