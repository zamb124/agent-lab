...
class TelegramInterface(BaseInterface):
    ...
    async def handle_message(self, raw_data: Dict[str, Any], flow_id:str) -> Optional[Message]:
        ...
        try:
            ...
            # Проверяем команды
            is_command, command_response = await self.process_command(text, chat_id, flow_id)
            if is_command:
    ...
    async def _extract_files_from_message(self, tg_message: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for file_type in file_types:
            if file_type in tg_message:
                ...
                files_data.append({
                    ...
                    'file_name': file_info.get('file_name', f"{file_type}_{file_info.get('file_id', 'unknown')}"),
    ...
    async def _process_single_file(self, file_data: Dict[str, Any], user_id: str, file_processor):
        ...
        try:
            ...
            logger.error(f"Ошибка обработки Telegram файла {file_data.get('file_id')}: {e}")
            return None
    ...
    async def _get_telegram_file_url(self, file_id: str) -> Optional[str]:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code != 200:
                    logger.error(f"Ошибка получения файла от Telegram: {response.status_code}")
                    return None
    ...
    @staticmethod
    async def get_bot_token_for_flow(flow_id: str, platform_config: Dict[str, Any]) -> Optional[str]:
