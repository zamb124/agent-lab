...
from app.core.models import TaskStatus, Context
...
class TaskProcessor:
    ...
    async def _process_single_task(self, task):
        ...
        logger.info(f"📨 {task.task_id}: '{user_msg}' | {task.context.user.name} → {task.flow_id}")
        ...
        try:
            ...
            try:
                ...
                logger.info(f"🔍 Состояние графа: next={state.next}, has_pending={has_pending}")
                ...
                if has_pending:
                    ...
                    logger.info(f"🔄 ЭТАП 1: Продолжаем прерванный tool_call с ответом пользователя")
                    ...
                    logger.info(f"🔄 ЭТАП 1 ЗАВЕРШЕН: tool_call обработан")
                else:
                    # Новый запрос
                ...
                logger.warning(f"⚠️ Не удалось получить состояние графа: {e}, выполняем как новый запрос")
                initial_state = {
                    "messages": [HumanMessage(content=user_message)],
                    **input_data_with_context
                }
            ...
            logger.info(f"🔍 Проверяем результат на interrupt: type={type(result)}, keys={list(result.keys()) if isinstance(result, dict) else 'НЕ DICT'}")
            ...
            if '__interrupt__' in result:
                ...
                logger.info(f"📤 Отправляем interrupt вопрос пользователю: {interrupt_value}")
                await self._send_result_via_interface(task, interrupt_value)
                
                return  # Выходим без завершения задачи
            else:
                logger.info("🔍 НЕТ __interrupt__ в результате, продолжаем обычное выполнение")
    ...
    async def _send_result_via_interface(self, task, result):
        ...
        logger.info(f"📤 {task.task_id}: '{response_text[:80]}{'...' if len(response_text) > 80 else ''}' → {task.platform}")
