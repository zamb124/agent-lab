...
async def send_message_to_flow(flow_id: str, request: FlowMessageRequest):
    ...
    if "api" not in flow_config.platforms:
        raise HTTPException(
            status_code=400, 
            detail=f"Флоу {flow_id} не поддерживает API платформу"
        )
    ...
    if interrupted_task:
        ...
        logger.info(f"🔄 Найдена прерванная задача {interrupted_task.task_id} для сессии {message.session_id}")
        
        # Обновляем прерванную задачу с новым сообщением пользователя
        logger.info(f"🔄 Обновляем задачу: старое сообщение='{interrupted_task.input_data.get('message', '')}', новое='{message.content}'")
        interrupted_task.input_data["message"] = message.content
        interrupted_task.input_data["metadata"] = message.metadata or {}
        logger.info(f"🔄 Переводим задачу {interrupted_task.task_id} из {interrupted_task.status} в pending")
        interrupted_task.status = TaskStatus.PENDING  # Возвращаем в pending для обработки
        
        await storage.set_task_config(interrupted_task)
        logger.info(f"🔄 Задача {interrupted_task.task_id} обновлена и сохранена в pending")
...
async def get_task_result(flow_id: str, task_id: str):
    ...
    try:
        ...
        logger.info(f"🔍 API читает задачу {task_id}: статус={task_info.get('status')}, output_data={bool(task_info.get('output_data'))}")
        if task_info.get('output_data'):
            output = task_info['output_data']
            logger.info(f"🔍 Output data: статус={output.get('status')}, вопрос_длина={len(output.get('question', ''))}")
        
        # Проверяем что задача относится к правильному флоу
        if task_info.get("flow_id") != flow_id:
            raise HTTPException(status_code=404, detail="Задача не найдена для этого флоу")
