...
async def telegram_webhook(flow_id: str, request: Request):
    ...
    try:
        ...
        if not telegram_config:
            ...
            raise HTTPException(status_code=400, detail=f"Flow {flow_id} does not support Telegram")
        ...
        logger.info(f"📨 Telegram webhook для flow {flow_id}: update {raw_data.get('update_id', 'unknown')}")
        ...
        logger.info(f"📋 Создана задача {task_id} для flow {flow_id} от пользователя {message.user_id}")
