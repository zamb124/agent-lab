...
from app.models import ConditionType, CodeMode
...
class ReactAgentRunner(BaseAgentRunner):
    ...
    async def arun(self, state: State) -> State:
        ...
        Returns:
            Финальное состояние после выполнения
        """
        max_iterations = state.get("remaining_steps", 25)
        ...
        while iteration < max_iterations:
            iteration += 1
            logger.debug(f"ReAct итерация {iteration}/{max_iterations}")
            ...
            if self.tools:
                ...
                logger.debug(f"Привязано {len(self.tools)} тулов к LLM")
            ...
            if not tool_calls:
                logger.info(f"ReAct завершен: LLM вернул финальный ответ (итерация {iteration})")
                break

            logger.info(f"Выполняем {len(tool_calls)} инструментов")
            ...
            try:
                ...
                if session_id:
                    ...
                    interrupt_ctx = state.get("interrupt_context", {})
                    if interrupt_ctx.get("type") != "tool_call":
                        state["interrupt_context"] = {
                            "type": "react_iteration",
                            "iteration": iteration
                        }
                        await state_manager.save_state(session_id, state)
                        logger.info(f"⏸️  ReAct: сохранена позиция итерации {iteration} при interrupt")
                raise interrupt
...
class StateGraphRunner(BaseAgentRunner):
    ...
    async def arun(self, state: State) -> State:
        ...
        while current_node and current_node != "END" and iteration < max_iterations:
            iteration += 1
            logger.info(f"StateGraph итерация {iteration}: выполняется нода {current_node}")

            node_func = self._node_functions.get(current_node)
            if not node_func:
                logger.error(f"Нода {current_node} не найдена в графе")
                break
            ...
            try:
                ...
                logger.info(f"Нода {current_node} выполнена, state keys: {list(state.keys())}, store keys: {list(state.get('store', {}).keys())}")
            except AgentInterrupt as interrupt:
                state["interrupt_context"] = {
            ...
            if not next_node:
                logger.warning(f"next_node is None или пустой для {current_node}, завершаем граф")
                break
            ...
            if cond_info["type"] == "ROUTER":
                ...
                if condition_func:
                    if asyncio.iscoroutinefunction(condition_func):
                        next_target = await condition_func(state)
                    else:
                        next_target = condition_func(state)
