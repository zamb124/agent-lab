...
class GithubProvider(BaseAuthProvider):
    ...
    async def exchange_code_for_token(
        ...
        # В GitHub допустимо и JSON, и form; для единообразия используем form
    ...
    async def get_user_info(self, access_token: str) -> Optional[ProviderUserInfo]:
        ...
        async with get_httpx_client(timeout=30.0, use_proxy_from_config=False) as client:
            ...
            if user_response.status_code != 200:
                ...
                raise ValueError(
                    f"Не удалось получить профиль GitHub: {user_response.status_code}"
                )
            user_data = user_response.json()

            email = user_data.get("email")
            if not email:
                # Если email скрыт в профиле, получаем первичный подтвержденный email
