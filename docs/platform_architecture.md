...
-   **Graph-based execution**: Вся логика работы агентов, их взаимодействие и управление состоянием строятся на основе графов состояний. Это позволяет использовать современные подходы к созданию мульти-агентных систем, включая циклы, ветвления и персистентность состояния.
...
-   **Расширяемость через Agent Development Kit (ADK)**: Платформа предоставляет четкий и простой API для разработчиков (ADK) для создания новых агентов и инструментов. ADK инкапсулирует сложность взаимодействия с раннерами агентов и брокером сообщений, позволяя разработчикам фокусироваться на бизнес-логике.

-   **Наблюдаемость (Observability)**: Каждый шаг выполнения агента, каждое событие и изменение состояния трассируется и логируется. Это обеспечивает полную прозрачность работы системы, что необходимо для отладки, мониторинга и анализа поведения автономных агентов. Платформа использует OpenTelemetry как стандарт для сбора данных трейсов, метрик и логов.
...
-   **Agent Core (Ядро агентов)**:
    -   **Раннеры (Runners)**: Отвечают за выполнение агентов. `ReActAgentRunner` для реактивных агентов и `StateGraphRunner` для графов состояний.
...
-   **State & Memory Layer (Слой состояния и памяти)**:
    -   **StateManager**: Механизм для сохранения и восстановления состояния агентов (PostgreSQL). Обеспечивает персистентность диалогов и возможность возобновления работы с прерванного места.
...
### 3.1. Графы состояний как ядро агентов

Ядром платформы является **графовая модель выполнения**. Все агенты, от простых реактивных до сложных супервизоров, выполняются через раннеры (Runners), которые управляют графом состояний.
...
-   **Динамическое возобновление (Interrupt Handling)**: Это ключевая особенность для интерактивного взаимодействия. Когда вложенному агенту требуется информация от пользователя, он вызывает `AgentInterrupt`.
    -   Прерывание "всплывает" вверх по стеку вызовов до Communication Layer.
    -   Система сохраняет полный state агента (включая ID прервавшегося агента) с помощью `StateManager`.
...
-   **Управление состоянием (State Management)**: Используется единый объект `State` (TypedDict), который передается по всему графу. Он содержит `messages`, `store` (общая память для данных) и другие служебные поля. `StateManager` автоматически сохраняет этот `State` после каждого шага, обеспечивая отказоустойчивость.
...
-   **`BaseAgent`**: Абстрактный класс, от которого наследуются все агенты. Он содержит базовую логику создания раннера и вызова (`ainvoke`).
...
-   **Механизм**:
    ...
    2.  Реализация этого инструмента не выполняет никаких внешних действий, а немедленно выбрасывает исключение `AgentInterrupt`.
    3.  Это прерывание "всплывает" вверх до `Communication Layer`, который приостанавливает выполнение агента и отправляет вопрос пользователю.
...
-   **Langfuse для анализа LLM**: В качестве основного бэкенда для анализа трейсов LLM-операций используется Langfuse. Он интегрируется с OpenTelemetry как специализированный `exporter` и предоставляет:
    -   Детализированную визуализацию выполнения графов состояний.
...
-   **Отказоустойчивость**: Kafka обеспечивает гарантированную доставку сообщений. Если воркер падает во время обработки задачи, сообщение не теряется и будет обработано другим воркером. `StateManager` обеспечивает возможность восстановления состояния агента после сбоя.
...
graph TD
    ...
    subgraph Agent Core
        ...
        Runner[Agent Runner] -.-> DB
    end

    subgraph Data & State
        direction LR
        DB[(Configuration DB)]
        StateManager[(State Manager: PostgreSQL)]
    end
    ...
    Factories -- Builds Agent --> Runner
    Runner -- Runs Graph --> Runner
    Runner -- find_agent_tool --> DB
    Runner -- Persists State --> StateManager
    Runner -- Returns Result --> Taskiq
...
sequenceDiagram
    ...
    Agent->>StateManager: 6. Сохраняет state после каждого шага
    Agent-->>Worker: 7. AgentInterrupt (нужен ввод)
    ...
    Worker->>StateManager: 14. Загружает сохраненный state
