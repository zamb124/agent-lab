...
- **Платформы**: Каналы, через которые Flow взаимодействует с пользователями (Telegram, API, Web, WhatsApp, AmoCRM).
...
```mermaid
graph LR
    A[Пользователь] --> B[Платформа (Telegram/API/Web)]
...
Платформа поддерживает гибкие варианты развертывания, включая Docker-контейнеры для изоляции и масштабирования.
