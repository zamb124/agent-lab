...
## 30.A) Lightpanda vs Chromium (CDP): глубинные различия и влияние на архитектуру

Этот раздел фиксирует **контрактные различия CDP-поведения** между Lightpanda и Chromium и их последствия для нашего Browser Runtime.

Опора на источники:

- Lightpanda CDP docs: `lightpanda.io/docs/cloud-offer/tools/cdp`
- Lightpanda CDP design note (координаты/DOM домен): `lightpanda.io/blog/posts/cdp-under-the-hood`
- Общая модель CDP (домены/таргеты/handlers): `chromium.googlesource.com/devtools/devtools-frontend/.../devtools-protocol.md`

### 30.A.1 CDP как “поверхность протокола” vs “семантика движка”

**CDP — это не “один API”, а набор доменов (Domain) и их методов/ивентов**, и разные движки могут:

- поддерживать **разные домены**;
- поддерживать домен, но **частично** (не все методы/поля/ивенты);
- реализовывать методы “формально совместимо”, но с **другой семантикой**.

Для архитектуры это означает, что “подключились по CDP” ещё не гарантирует:

- наличие полноценной модели рендера/лейаута;
- корректность viewport-зависимых действий (клики/скролл/хит-тест);
- работоспособность `networkidle` и тонкой сетевой телеметрии;
- поддержку PDF/скриншотов и всего, что требует графического пайплайна.

### 30.A.2 Ключевая ось отличий: наличие полноценного рендеринга

Lightpanda позиционируется как headless-движок “для машин”, с упором на скорость/память и CDP-совместимость. При этом **у него нет цели быть “полным Chromium”**.

Практическая разница:

- **Chromium CDP** “естественно” опирается на движок рендера (layout/paint), поэтому координаты и hit testing имеют пиксельный смысл.
- **Lightpanda CDP** может эмулировать координатные действия без настоящего pixel-based layout. В их описании используется подход, где “координаты” — это **идентификатор** в плоском представлении (а не реальные пиксели).

Следствие: все операции, которые в автоматизации неявно зависят от “реального viewport”, становятся зоной риска при одинаковом CDP-wire-формате.

### 30.A.3 Что это значит для нашего текущего Browser Runtime (реальное использование)

Текущая реализация `apps/browser` использует Playwright поверх `chromium.connect_over_cdp(...)` и строит контрольные операции через API Playwright:

- **CDP transport**:
  - один Browser на endpoint (`CDPConnectionPool`);
  - выдача `BrowserContext/page` через `PageLeaseManager` и `ContextFactory`.
- **Навигация**:
  - `page.goto(..., wait_until="domcontentloaded"|"networkidle")` и вариант `selector:<css>`.
- **Интеракции**:
  - `locator.click()` + `force=True` как last-resort;
  - “human-like” `page.mouse.move(...)`, `page.mouse.wheel(...)`, `page.keyboard.press(...)`;
  - строго refs → `(role, name, nth)` → `page.get_by_role(...).nth(n)`.
- **Observe / snapshot**:
  - интерактивный snapshot строится **через `page.evaluate(...)`** (выгрузка интерактивных узлов и их ролей/имён);
  - HTML sidecar берётся `page.content()`.
- **Артефакты**:
  - PDF: `page.pdf(...)` (явная зависимость от рендеринга);
  - screenshot в fetch сейчас не снимается (плейсхолдер), но флаг уже есть в контракте.

### 30.A.4 Что мы сможем переиспользовать при внедрении Lightpanda (почти 1:1)

Ниже перечислено то, что **не зависит** от “полного Chromium”, а зависит только от базового CDP + базового Playwright API:

- **Контракты control API**: `acquire/navigate/observe/click/fill/press/wait/close_session` и их DTO (`BrowserAcquireRequest`, `BrowserFetchRequest`, refs snapshot).
- **Сессионная модель**:
  - `CDPConnectionPool` (один Playwright driver + один Browser на endpoint);
  - `PageLeaseManager` (lease/TTL, session_id → один context, “one page per session” инвариант control API);
  - `CDPLifecycleManagerImpl` (drain/disconnect на endpoint уровне).
- **Observe слой (LLM-friendly snapshot)**:
  - подход “snapshot через `page.evaluate`” остаётся валидным и даже предпочтительным (Lightpanda в своих материалах подчёркивает, что для extraction automation библиотеки часто выбирают Runtime/evaluate вместо тяжёлого DOM домена).
- **Refs-only interaction contract**:
  - “действовать только по refs из snapshot” снижает зависимость от CSS/XPath и делает поведение более переносимым между движками.
- **Запись console/network сигналов через события Playwright**:
  - `page.on("console"|"pageerror"|"requestfailed"|"response")` — если Playwright корректно транслирует их через CDP у Lightpanda, эта часть архитектуры переиспользуется.

### 30.A.5 Что НЕ переиспользуется (или должно стать capability-gated)

Это список областей, где **Chromium-специфичная семантика** или “полный рендеринг” критичны. Их нельзя считать доступными в Lightpanda “по умолчанию”, даже если подключение по CDP работает.

- **PDF (`page.pdf`)**:
  - требует полноценного layout/print pipeline; для Lightpanda это почти наверняка не-каноничный путь.
  - В нашей архитектуре `capture_pdf` должен трактоваться как **опциональная capability** (иначе 501 `BrowserCapabilityError`).
- **Скриншоты (`page.screenshot` / `Page.captureScreenshot`)**:
  - если Lightpanda реализует “flat rendering”, скриншот может быть отсутствующим, упрощённым или несовместимым по ожиданиям.
  - Следствие: `screenshot` — тоже capability, а не обязательный артефакт.
- **Viewport- и координатно-зависимые интеракции**:
  - `mouse.move`, `mouse.wheel`, “outside of the viewport” ошибки, hit target, sticky headers — всё это сильно зависит от того, что движок считает viewport-координатами.
  - В Lightpanda “координаты как идентификатор” могут ломать допущения Playwright (например, стратегии, которые сначала вычисляют bounding box, потом кликают).
- **`networkidle`**:
  - в control API уже отмечено, что `networkidle` может не наступать (и observe не должен зависеть от него).
  - Для Lightpanda это нужно фиксировать как риск: стратегию ожидания лучше строить на `domcontentloaded` + `selector:...` (и/или доменно-специфичных condition checks).
- **Расширенные домены CDP (производительность, трейсинг, эммуляция, overlay, глубокие network events)**:
  - даже при заявленной CDP-совместимости покрытие доменов может быть частичным.
  - Любые “профилировочные” или “девтулзные” фичи нельзя делать обязательными для runtime-контракта.

### 30.A.6 Архитектурная рекомендация: capability-first вместо “fallback на Chromium”

Так как оба движка подключаются по CDP, правильная граница ответственности должна быть такой:

- **Контракт control API остаётся стабильным**, но каждая “дорогая/рендерная” фича — опциональна.
- **Не делаем неявный fallback на Chromium в коде** (zero-guess): если capability отсутствует — операция падает с `BrowserCapabilityError` (HTTP 501) и это фиксируется в артефактах run.
- “Fallback на Chromium” допустим только как **явный выбор endpoint-а** orchestration-слоем (policy/scheduler), а не как скрытая деградация внутри interactor.
