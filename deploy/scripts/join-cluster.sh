...
#
# Что делает:
#   1. Если gpu-worker уже в кластере → skip всех шагов join.
#   2. microk8s add-node — выпускает разовый токен.
#   3. SSH на gpu-worker → microk8s join <token>.
#   4. Дожидается gpu-worker Ready.
#   5. kubectl label node gpu-worker accelerator=nvidia-gpu (с --overwrite).
#   6. SSH на gpu-worker → microk8s enable gpu (NVIDIA Device Plugin).
#   7. Проверяет, что nvidia.com/gpu виден в Allocatable ноды.
#
# ENV:
...
# 5. Лейбл (overwrite — идемпотентно)
...
# 6. enable gpu на worker (нужно делать на той ноде, где GPU физически)
log_do "SSH $GPU_WORKER_HOST: microk8s enable gpu"
if ! ssh -o BatchMode=yes "$GPU_WORKER_HOST" "microk8s enable gpu" ; then
  log_warn "microk8s enable gpu вернул ошибку — возможно уже включено. Продолжаем."
fi

# 7. Проверяем, что NVIDIA Device Plugin видит GPU
wait_for \
  "node $GPU_NODE_NAME имеет nvidia.com/gpu в Allocatable" \
  "microk8s kubectl get node $GPU_NODE_NAME -o jsonpath='{.status.allocatable.nvidia\\.com/gpu}' 2>/dev/null | grep -qE '^[1-9]'" \
  300 10 \
  || log_warn "GPU ещё не доступна. Проверьте: microk8s kubectl describe node $GPU_NODE_NAME"
