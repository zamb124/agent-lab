...
def log_llm_stream_response(
    *,
    ...
    raw_input_tokens = usage.get("input_tokens") or usage.get("prompt_tokens") or 0
    raw_output_tokens = usage.get("output_tokens") or usage.get("completion_tokens") or 0
    input_tokens = (
    ...
    output_tokens = (
        ...
        if isinstance(raw_output_tokens, (int, float)) and not isinstance(raw_output_tokens, bool)
        else 0
    )
    raw_total_tokens = usage.get("total_tokens")
    total_tokens = (
        int(raw_total_tokens)
        if isinstance(raw_total_tokens, (int, float)) and not isinstance(raw_total_tokens, bool)
