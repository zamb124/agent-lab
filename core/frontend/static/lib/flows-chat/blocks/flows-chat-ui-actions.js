...
export class FlowsChatUiActions extends LitElement {
    static properties = {
        buttons: { type: Array },
    };
    ...
    static styles = css`
        ...
        button {
            ...
            font-weight: 500;
            cursor: pointer;
            border: 1px solid var(--flows-chat-border, rgba(255, 255, 255, 0.2));
            background: var(--flows-chat-accent-muted, rgba(153, 166, 249, 0.2));
            color: var(--flows-chat-text, rgba(255, 255, 255, 0.92));
        }
        button:hover {
            background: var(--flows-chat-accent, rgba(153, 166, 249, 0.35));
        }
    `;
    ...
    _clearNativeHandlers() {
        ...
        root.querySelectorAll('button[data-flows-chat-action-index]').forEach((n) => {
    ...
    _wireNativeButtonHandlers() {
        ...
        root.querySelectorAll('button[data-flows-chat-action-index]').forEach((node) => {
            if (!(node instanceof HTMLButtonElement) || node.type !== 'button' || node.disabled) {
                return;
            }
            const raw = node.getAttribute('data-flows-chat-action-index');
    ...
    _onClick(btn) {
        ...
        if (!actionId || !actionKind) {
            this.dispatchEvent(
                new CustomEvent('flows-chat-action-config-error', {
                    bubbles: true,
                    composed: true,
                    detail: {
    ...
    render() {
        ...
        return html`
            ${btns.map(
                (b, i) => html`
                    <button type="button" data-flows-chat-action-index=${String(i)}>
...
customElements.define('flows-chat-ui-actions', FlowsChatUiActions);
