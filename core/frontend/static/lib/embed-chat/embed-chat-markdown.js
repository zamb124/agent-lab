/**
 * Ответы ассистента в embed-чате: полный GFM через `globalThis.marked` (скрипт `/static/core/assets/js/marked.min.js`).
 ...
 * Во время A2A-стрима (`embedAssistantMarkdownToHtml(..., { streaming: true })`) используется только этот fallback — без тяжёлого `marked.parse`, чтобы чанки успевали отображаться; после завершения стрима вызывать с `streaming: false` для полного GFM.
 */
 ...
 * @returns {string} HTML-фрагмент
 */
export function embedAssistantMarkdownToHtml(markdownText, opts) {
