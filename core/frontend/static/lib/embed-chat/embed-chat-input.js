...
export class EmbedChatInput extends LitElement {
    ...
    constructor() {
        ...
        this._onTtsPrefEmbedInput = () => {
            this.requestUpdate();
        };
        this._onTtsStorageEmbedInput = (e) => {
    ...
    connectedCallback() {
        ...
        if (typeof window !== 'undefined') {
            window.addEventListener(TTS_OUTPUT_CHANGED_EVENT, this._onTtsPrefEmbedInput);
            window.addEventListener('storage', this._onTtsStorageEmbedInput);
        }
    }

    disconnectedCallback() {
        if (typeof window !== 'undefined') {
            window.removeEventListener(TTS_OUTPUT_CHANGED_EVENT, this._onTtsPrefEmbedInput);
            window.removeEventListener('storage', this._onTtsStorageEmbedInput);
        }
        super.disconnectedCallback();
    }

    _onTtsOutputEmbedClick() {
        toggleTtsOutputEnabled();
        this.requestUpdate();
    }
    ...
    render() {
        ...
        const ttsOutOn = readTtsOutputEnabled();
        const ttsVolOnSvg = html`<svg width="20" height="20" viewBox="0 0 24 24" fill="none" aria-hidden="true">
            <path
                d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z"
                fill="currentColor"
            />
        </svg>`;
        const ttsVolOffSvg = html`<svg width="20" height="20" viewBox="0 0 24 24" fill="none" aria-hidden="true">
            <path
            ...
            <div class="composer">
                ...
                ${this._composerMicVisible() === true
                    ? html`
                          <button
                              type="button"
                              class="circle-btn ${ttsOutOn ? 'active' : ''}"
                              ?disabled=${this.loading}
                              @click=${this._onTtsOutputEmbedClick}
                              title=${ttsOutOn
                                  ? this._label('tts_output_disable', 'Disable spoken responses')
                                  : this._label('tts_output_enable', 'Enable spoken responses')}
                              aria-label=${ttsOutOn
                                  ? this._label('tts_output_disable', 'Disable spoken responses')
                                  : this._label('tts_output_enable', 'Enable spoken responses')}
                              aria-pressed=${ttsOutOn ? 'true' : 'false'}
                          >
                              ${ttsOutOn ? ttsVolOnSvg : ttsVolOffSvg}
                          </button>
                          <button
                              type="button"
