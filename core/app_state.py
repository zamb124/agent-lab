...
def require_request_trace_id(request: Request) -> str:
    trace_id = getattr(request.state, "trace_id", None)
    if not isinstance(trace_id, str) or not trace_id.strip():
        raise RuntimeError(
            "request.state.trace_id отсутствует. AccessLogMiddleware должен быть внешним слоем."
        )
    return trace_id
...
def get_request_company_id(request: Request) -> str | None:
    ...
    if not isinstance(company, Company):
        raise RuntimeError("request.state.company имеет неожиданный тип")
...
def get_request_token_data(request: Request) -> TokenData | None:
    ...
    if not isinstance(token_data, TokenData):
        raise RuntimeError("request.state.token_data имеет неожиданный тип")
    return token_data
...
def read_session_token_override(request: Request) -> TokenData | None:
    session_token_data = getattr(request.state, "session_token_data", None)
    if session_token_data is None:
        return None
    if not isinstance(session_token_data, TokenData):
        raise RuntimeError("request.state.session_token_data имеет неожиданный тип")
    return session_token_data


def read_reissue_auth_token(request: Request) -> str | None:
    reissue_auth_token = getattr(request.state, "reissue_auth_token", None)
    if not isinstance(reissue_auth_token, str) or not reissue_auth_token.strip():
        return None
    return reissue_auth_token
