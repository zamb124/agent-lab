...
**Sync Service** (`tests/sync/`): только реальные фикстуры — PostgreSQL, Redis, TaskIQ worker (для `messages.transcribe_*` и `sync_finalize_recording_task`). Нельзя подменять `op_*` функции из `apps/sync/realtime/operations.py`, репозитории или WebSocket через `unittest.mock` / `monkeypatch`. **Единственное явно разрешённое исключение** — `@patch("core.push.service.webpush")` в `tests/sync/test_sync_notification_delivery.py`: Web Push к Apple/FCM требует device tokens и VAPID-ключей, физически невозможно в CI. Подробности: `.cursor/rules/sync.mdc`, `tests/sync/README.md`. Изоляция тестов — через уникальный `company_id` и фикстуру `unique_id` для всех entity-id; фикстура `sync_db_clean` — **no-op** (БД между тестами не очищается, опираемся на уникальные id). Все тесты под `tests/sync/` автоматически в **`xdist_group("sync_db")`** — при `-n` и `--dist=loadgroup` выполняются **на одном gw последовательно**, без параллельной гонки за одной схемой. Календарные ссылки (`calendar_event_id`, канал `calendar_meeting`, `GET /sync/api/v1/calls/links/scheduled`, PATCH состава участников) — **`tests/sync/api/test_calendar_sync_call_links.py`**. Сквозной сценарий «frontend календарь, `kind=meeting`, реальный Sync HTTP» — **`tests/core/api/test_calendar_sync_meeting_integration.py`** (фикстура **`sync_service`**, порт **9005**; для join URL в тесте выставляется **`SERVER__PLATFORM_PUBLIC_BASE_URL`**).
...
| Изоляция фабрик и bus | Между тестами вызывать **`clearFactoryRegistry()`** ([`@platform/lib/events/factory-registry.js`](../../core/frontend/static/lib/events/factory-registry.js)), **`_resetResourceRegistryForTests()`** и **`resetPlatformBusForTests()`** ([`@platform/lib/events/index.js`](../../core/frontend/static/lib/events/index.js)). Не использовать в прод-коде. |
...
- **Sync UI:** сценарии в `tests/ui/e2e/test_sync_*.py`; хелперы `tests/ui/e2e/sync_e2e_helpers.py` (локаторы сайдбара без коллизии с шапкой чата, при необходимости вызовы Sync REST через `httpx` с cookie `auth_token`). Теги сценариев см. `sync.mdc` (раздел «Тестирование»). Упоминания и карточка профиля: `test_sync_mentions.py` (клик по `.msg-mention--interactive`, core-модалка `platform.user_info` через `<platform-user-info-modal>`, общие каналы, выбор канала из сетки); профиль из аватарки чужого сообщения — `test_sync_settings_and_profile.py`.
